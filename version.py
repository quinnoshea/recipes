...
try:
    ...
    try:
        tandoor_tag = (
            subprocess.check_output(
                ["git", "describe", "--exact-match", "--tags", tandoor_hash],
                cwd=BASE_DIR,
            )
            .decode()
            .replace("\n", "")
    ...
    version_info.append(
        {
            ...
            "commit_link": "https://github.com/TandoorRecipes/recipes/commit/"
            + r.split("\n")[0].split(" ")[1],
            "ref": tandoor_hash,
        ...
        for d in os.listdir(PLUGINS_DIRECTORY):
            if d != "__pycache__":
                try:
                    ...
                    print(
                        "getting plugin version for ",
                        (
                            plugin_class.verbose_name
                    ...
                    r = subprocess.check_output(
                        ["git", "show", "-s"],
                        cwd=os.path.join(BASE_DIR, "recipes", "plugins", d),
                    ).decode()
                    ...
                    try:
                        ...
                        tag = (
                            subprocess.check_output(
                                ["git", "describe", "--exact-match", commit_hash],
                                cwd=os.path.join(BASE_DIR, "recipes", "plugins", d),
                            )
                            .decode()
                            .replace("\n", "")
                    ...
                    version_info.append(
                        {
                            "name": (
                                "Plugin: " + plugin_class.verbose_name
                                if hasattr(plugin_class, "verbose_name")
                                else plugin_class.name
                            ),
                            "version": re.sub(r"<.*>", "", r),
                            "website": (
                                plugin_class.website

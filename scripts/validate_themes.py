...
def validate_theme_file(filepath: Path) -> List[str]:
    ...
    if '--color-background' in colors and '--color-text-primary' in colors:
        ratio = contrast_ratio(colors['--color-background'], colors['--color-text-primary'])
        if ratio < 4.5:
            errors.append(f"Poor text contrast ratio: {ratio:.2f} (minimum 4.5 required)")
    ...
    if '--color-primary' in colors and '--color-on-primary' in colors:
        ...
        if ratio < 4.5:
            errors.append(f"Poor primary button contrast: {ratio:.2f} (minimum 4.5 required)")

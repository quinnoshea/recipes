...
class StepAdmin(admin.ModelAdmin):
    ...
    def recipe_and_name(obj):
        ...
        return (
            f"{obj.recipe_set.first().name}: {obj.name}"
            if obj.name
            else obj.recipe_set.first().name
        )
...
def rebuild_index(modeladmin, request, queryset):
    ...
    with scopes_disabled():
        Recipe.objects.all().update(
            name_search_vector=SearchVector(
                "name__unaccent", weight="A", config=language
            ),
            desc_search_vector=SearchVector(
                "description__unaccent", weight="B", config=language
            ),
        )
...
class IngredientAdmin(admin.ModelAdmin):
    ...
    def recipe_name(obj):
        recipes = (
            obj.step_set.first().recipe_set.all() if obj.step_set.exists() else None
        )

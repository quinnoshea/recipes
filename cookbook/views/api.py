...
from cookbook.serializer import (
    ...
    GroupSerializer,
    ImportLogSerializer,
    ImportOpenDataMetaDataSerializer,
    ImportOpenDataResponseSerializer,
    ImportOpenDataSerializer,
    ...
    ShoppingListEntryBulkCreateSerializer,
...
from recipes.settings import (
    AI_RATELIMIT,
    DRF_THROTTLE_RECIPE_URL_IMPORT,
    FDC_API_KEY,
)

DateExample = OpenApiExample("Date Format", value="1972-12-05", request_only=True)
BeforeDateExample = OpenApiExample(
    "Before Date Format", value="-1972-12-05", request_only=True
)
...
@extend_schema_view(
    list=extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                name="limit", description="limit number of entries to return", type=str
            ),
            OpenApiParameter(
                name="random",
    ...
    def get_queryset(self):
        ...
        if root:
            ...
            if root_tree.isnumeric():
                try:
                    self.queryset = (
                        self.model.objects.get(id=int(root_tree))
                        .get_root()
                        .get_descendants_and_self()
                    )
                except self.model.DoesNotExist:
...
class SpaceViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return self.queryset.filter(
            id__in=UserSpace.objects.filter(user=self.request.user).values_list(
                "space_id", flat=True
            )
        )
    ...
    def current(self, request):
        ...
        return Response(
            self.serializer_class(
                self.request.space, many=False, context={"request": self.request}
            ).data
        )
...
class UserPreferenceViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        with (
            scopes_disabled()
        ):  # need to disable scopes as user preferences are not scoped
...
class SearchPreferenceViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        with (
            scopes_disabled()
        ):  # need to disable scopes as search preferences are not scoped
            return self.queryset.filter(user=self.request.user)
...
class AiProviderViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        with scopes_disabled():
            return self.queryset.filter(
                Q(space=self.request.space) | Q(space__isnull=True)
            )
...
class SyncViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def query_synced_folder(self, request, pk):
        ...
        return Response(
            SyncLogSerializer(
                sync_log, many=False, context={"request": self.request}
            ).data
        )
...
class RecipeImportViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def import_recipe(self, request, pk):
        ...
        return Response(
            RecipeSerializer(recipe, many=False, context={"request": self.request}).data
        )
...
class KeywordViewSet(LoggingMixin, TreeMixin):
    ...
    permission_classes = [
        (CustomIsGuest & IsReadOnlyDRF | CustomIsUser) & CustomTokenHasReadWriteScope
    ]
    pagination_class = DefaultPagination
...
class FoodViewSet(LoggingMixin, TreeMixin):
    ...
    def fdc(self, request, pk):
        ...
        if not food.fdc_id:
            return JsonResponse(
                {"msg": "Food has no FDC ID associated."},
                status=400,
                json_dumps_params={"indent": 4},
            )

        response = requests.get(
            f"https://api.nal.usda.gov/fdc/v1/food/{food.fdc_id}?api_key={FDC_API_KEY}"
        )
        if response.status_code == 429:
            return JsonResponse(
                {
                    "msg": "API Key Rate Limit reached/exceeded, see https://api.data.gov/docs/rate-limits/ for more information. \
        ...
        standard_unit = Unit.objects.filter(
            base_unit__iexact="g", space=self.request.space
        ).first()
        if not standard_unit:
            standard_unit = Unit.objects.filter(
                name__iexact="g", space=self.request.space
            ).first()
            if not standard_unit:
                standard_unit = Unit.objects.create(
                    name="g", base_unit="g", space=self.request.space
                )
            else:
        ...
        try:
            ...
            for pt in PropertyType.objects.filter(
                space=request.space, fdc_id__gte=0
            ).all():
                if pt.fdc_id:
            ...
            for p in properties:
                property_food_relation_list.append(
                    Food.properties.through(food_id=food.id, property_id=p.pk)
                )

            FoodProperty.objects.bulk_create(
                property_food_relation_list,
            ...
            return JsonResponse(
                {
                    "msg": "there was an error parsing the FDC data, please check the server logs"
                },
                status=500,
                json_dumps_params={"indent": 4},
            )
...
MealPlanViewQueryParameters = [
    OpenApiParameter(
        name="from_date",
        description=_("Filter meal plans from date (inclusive)."),
        type=str,
        examples=[DateExample],
    ),
    OpenApiParameter(
        name="to_date",
        description=_("Filter meal plans to date (inclusive)."),
        type=str,
        examples=[DateExample],
    ),
    OpenApiParameter(
        name="meal_type",
        description=_(
            "Filter meal plans with MealType ID. For multiple repeat parameter."
        ),
        type=str,
        many=True,
    ),
...
class MealPlanViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def ical(self, request):
        from_date = self.request.query_params.get("from_date", None)
        to_date = self.request.query_params.get("to_date", None)
        return meal_plans_to_ical(
            self.get_queryset(), f"meal_plan_{from_date}-{to_date}.ics"
        )
...
class MealTypeViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = (
            self.queryset.order_by("time", "id")
...
@extend_schema_view(
    list=extend_schema(
        parameters=[
            OpenApiParameter(
                name="food", description="ID of food to filter for", type=int
            ),
            OpenApiParameter(
                name="unit", description="ID of unit to filter for", type=int
            ),
        ]
    )
)
...
@extend_schema_view(
    ...
    list=extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                name="units",
                description=_("ID of unit a recipe should have."),
                type=int,
            ),
            OpenApiParameter(
                name="rating", description=_("Exact rating of recipe"), type=int
            ),
            OpenApiParameter(
                name="rating_gte",
                description=_("Rating a recipe should have or greater."),
                type=int,
            ),
            OpenApiParameter(
                name="rating_lte",
                description=_("Rating a recipe should have or smaller."),
                type=int,
            ),
            OpenApiParameter(
                name="timescooked",
                description=_("Filter recipes cooked X times."),
                type=int,
            ),
            OpenApiParameter(
                name="timescooked_gte",
                description=_("Filter recipes cooked X times or more."),
                type=int,
            ),
            OpenApiParameter(
                name="timescooked_lte",
                description=_("Filter recipes cooked X times or less."),
                type=int,
            ),
            OpenApiParameter(
                name="createdon",
            ...
            OpenApiParameter(
                name="cookedon_gte",
                description=_("Filter recipes last cooked on the given date or after."),
                type=OpenApiTypes.DATE,
            ),
            OpenApiParameter(
                name="cookedon_lte",
                description=_(
                    "Filter recipes last cooked on the given date or before."
                ),
                type=OpenApiTypes.DATE,
            ),
            OpenApiParameter(
            ...
            OpenApiParameter(
                name="num_recent",
                description=_(
                    "Returns the given number of recently viewed recipes before search results (if given)"
                ),
                type=int,
            ),
            OpenApiParameter(
                name="filter",
                description=_(
                    "ID of a custom filter. Returns all recipes matched by that filter."
                ),
                type=int,
            ),
            OpenApiParameter(
...
class RecipeViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def flat(self, request):
        ...
        qs = (
            Recipe.objects.filter(space=request.space)
            .filter(
                Q(private=False)
                | (
                    Q(private=True)
    ...
    @decorators.action(
        detail=False, methods=["PUT"], serializer_class=RecipeBatchUpdateSerializer
    )
    def batch_update(self, request):
        ...
        if serializer.is_valid():
            recipes = Recipe.objects.filter(
                id__in=serializer.validated_data["recipes"], space=self.request.space
            )
            safe_recipe_ids = Recipe.objects.filter(
                id__in=serializer.validated_data["recipes"], space=self.request.space
                ...
                for r in recipes:
                    for k in serializer.validated_data["keywords_add"]:
                        keyword_relations.append(
                            Recipe.keywords.through(recipe_id=r.pk, keyword_id=k)
                        )
                Recipe.keywords.through.objects.bulk_create(
                    keyword_relations,
                    ignore_conflicts=True,
                    unique_fields=(
                        "recipe_id",
            ...
            if "keywords_remove" in serializer.validated_data:
                for k in serializer.validated_data["keywords_remove"]:
                    Recipe.keywords.through.objects.filter(
                        recipe_id__in=safe_recipe_ids, keyword_id=k
                    ).delete()

            if (
                "keywords_set" in serializer.validated_data
                and len(serializer.validated_data["keywords_set"]) > 0
            ):
                keyword_relations = []
                Recipe.keywords.through.objects.filter(
                    recipe_id__in=safe_recipe_ids
                ).delete()
                for r in recipes:
            ...
            if (
                "keywords_remove_all" in serializer.validated_data
                and serializer.validated_data["keywords_remove_all"]
            ):
                Recipe.keywords.through.objects.filter(
                    recipe_id__in=safe_recipe_ids
                ).delete()
            ...
            if (
                "private" in serializer.validated_data
                and serializer.validated_data["private"] is not None
            ):
            ...
            if "shared_add" in serializer.validated_data:
                ...
                for r in recipes:
                    for u in serializer.validated_data["shared_add"]:
                        shared_relation.append(
                            Recipe.shared.through(recipe_id=r.pk, user_id=u)
                        )
                Recipe.shared.through.objects.bulk_create(
                    shared_relation,
                    ignore_conflicts=True,
                    unique_fields=(
                        "recipe_id",
            ...
            if "shared_remove" in serializer.validated_data:
                for s in serializer.validated_data["shared_remove"]:
                    Recipe.shared.through.objects.filter(
                        recipe_id__in=safe_recipe_ids, user_id=s
                    ).delete()

            if (
                "shared_set" in serializer.validated_data
                and len(serializer.validated_data["shared_set"]) > 0
            ):
                shared_relation = []
                Recipe.shared.through.objects.filter(
                    recipe_id__in=safe_recipe_ids
            ...
            if (
                "shared_remove_all" in serializer.validated_data
                and serializer.validated_data["shared_remove_all"]
            ):
                Recipe.shared.through.objects.filter(
                    recipe_id__in=safe_recipe_ids
                ).delete()
            ...
            if (
                "clear_description" in serializer.validated_data
                and serializer.validated_data["clear_description"]
            ):
                recipes.update(description="")

            if (
                "show_ingredient_overview" in serializer.validated_data
                and serializer.validated_data["show_ingredient_overview"] is not None
            ):
                recipes.update(
                    show_ingredient_overview=serializer.validated_data[
    ...
    def delete_external(self, request, pk):
        obj = self.get_object()
        if obj.get_space() != request.space and has_group_permission(
            request.user, ["user"]
        ):
            raise PermissionDenied(
                detail="You do not have the required permission to perform this action",
        ...
        return Response(
            self.serializer_class(obj, many=False, context={"request": request}).data
        )
...
@extend_schema_view(
    list=extend_schema(
        parameters=[
            OpenApiParameter(
                name="food_id", description="ID of food to filter for", type=int
            ),
            OpenApiParameter(
                name="query",
    ...
    def get_queryset(self):
        ...
        if query is not None:
            self.queryset = self.queryset.filter(
                ...
                | Q(base_unit__name__icontains=query)
                | Q(converted_unit__name__icontains=query)
            )
    ...
    list=extend_schema(
        parameters=[
            OpenApiParameter(
                name="category",
                description=_(
                    "Return the PropertyTypes matching the property category.  Repeat for multiple."
                ),
                type=str,
                many=True,
    ...
    list=extend_schema(
        parameters=[
            OpenApiParameter(
                name="mealplan",
                description=_(
                    "Returns only entries associated with the given mealplan id"
                ),
                type=int,
            )
        ]
...
class ShoppingListRecipeViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        self.queryset = self.queryset.filter(
            ...
            | Q(recipe__space=self.request.space)
            | Q(mealplan__space=self.request.space)
        )
    ...
    @decorators.action(
        detail=True,
        methods=["POST"],
        serializer_class=ShoppingListEntryBulkCreateSerializer,
...
class ShoppingListEntryViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not self.detail:
            ...
            self.queryset = self.queryset.filter(
                (Q(checked=False) | Q(completed_at__gte=week_ago))
            )
        ...
        if serializer.is_valid():
            ...
            if checked:
                bulk_entries.update(
                    ...
                    completed_at=update_timestamp,
                )
            else:
                bulk_entries.update(
                    checked=checked, updated_at=update_timestamp, completed_at=None
                )
            ...
            if request.user.userpreference.shopping_add_onhand:
                ...
                if checked:
                    for f in foods:
                        f.onhand_users.add(
                            *request.user.userpreference.shopping_share.all(),
                            request.user,
                        )
                elif checked == False:
                    for f in foods:
                        f.onhand_users.remove(
                            *request.user.userpreference.shopping_share.all(),
                            request.user,
                        )
    ...
    list=extend_schema(
        parameters=[
            OpenApiParameter(
                name="internal_note",
                description=_(
                    "Text field to store data that gets carried over to the UserSpace created from the InviteLink"
                ),
                type=str,
            ),
            OpenApiParameter(
                name="unused",
                description=_("Only return InviteLinks that have not been used yet."),
                type=bool,
            ),
        ]
    )
    ...
    list=extend_schema(
        parameters=[
            OpenApiParameter(
                name="type",
                description=_(
                    "Return the CustomFilters matching the model type.  Repeat for multiple."
                ),
                type=str,
                many=True,
...
class RecipeUrlImportView(APIView):
    ...
    @extend_schema(
        request=RecipeFromSourceSerializer(many=False),
        responses=RecipeFromSourceResponseSerializer(many=False),
    )
    def post(self, request, *args, **kwargs):
        """
        ...
        if serializer.is_valid():
            ...
            if not url and not data:
                ...
                return Response(
                    RecipeFromSourceResponseSerializer().to_representation(response),
                ...
                if re.match("^(https?://)?(www\\.youtube\\.com|youtu\\.be)/.+$", url):
                    if validate_import_url(url):
                        ...
                        if url and url.strip() != "":
                            response["duplicates"] = (
                                Recipe.objects.filter(
                                    space=request.space, source_url=url.strip()
                                )
                                .values("id", "name")
                                .all()
                            )
                        return Response(
                            ...
                            ).to_representation(response),
                            status=status.HTTP_200_OK,
                        )

                tandoor_url = None
                if re.match(
                    r"^(.)*/recipe/[0-9]+/\?share=[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$",
                    url,
                ):
                    tandoor_url = url.replace("/recipe/", "/api/recipe/")
                elif re.match(
                    r"^(.)*/view/recipe/[0-9]+/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$",
                    url,
                ):
                    tandoor_url = (
                        url.replace("/view/recipe/", "/api/recipe/").replace(
                            re.split("/recipe/[0-9]+", url)[1], ""
                        )
                        + "?share="
                ...
                if tandoor_url and validate_import_url(tandoor_url):
                    ...
                    try:
                        if validate_import_url(url):
                            ...
                            scrape = scrape_html(
                                org_url=url, html=html, supported_only=False
                            )
                        else:
                            response["error"] = True
                            response["msg"] = _("Invalid Url")
                            return Response(
                                RecipeFromSourceResponseSerializer().to_representation(
                                    response
                                ),
                                status=status.HTTP_400_BAD_REQUEST,
                            )
                ...
                try:
                    ...
                    pass
                scrape = scrape_html(
                    html=data, org_url="https://urlnotfound.none", supported_only=False
                )
                if not url and (
                    found_url := scrape.schema.data.get(
                        "url", "https://urlnotfound.none"
                    )
                ):
                    scrape = scrape_html(
                        html=data, org_url=found_url, supported_only=False
                    )
...
class AiImportView(APIView):
    ...
    @extend_schema(
        request=AiImportSerializer(many=False),
        responses=RecipeFromSourceResponseSerializer(many=False),
    )
    def post(self, request, *args, **kwargs):
        """
        ...
        if serializer.is_valid():
            ...
            ai_provider = (
                AiProvider.objects.filter(
                    pk=serializer.validated_data["ai_provider_id"]
                )
                .filter(Q(space=request.space) | Q(space__isnull=True))
                .first()
            )
            ...
            if serializer.validated_data["recipe_id"]:
                if recipe := Recipe.objects.filter(
                ...
                messages = [
                    {
                        ...
                        "content": [
                            ...
                            {"type": "text", "text": serializer.validated_data["text"]},
                        ],
                    },
                ]
            ...
            try:
                ...
                if scrape:
                    ...
                    obj, created = Keyword.objects.get_or_create(
                        name="✨ AI", space=request.space
                    )
                    recipe["keywords"].append(
                        {
                            "label": obj.name,
                            "name": obj.name,
                            "id": obj.id,
...
class AppImportView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if limit:
            return Response(
                {"error": True, "msg": _("File is above space limit")},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class AppExportView(APIView):
    ...
    @extend_schema(
        request=ExportRequestSerializer(many=False),
        responses=ExportLogSerializer(many=False),
    )
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            if serializer.validated_data["all"]:
                recipes = Recipe.objects.filter(
                    space=request.space, internal=True
                ).all()
            elif serializer.validated_data["custom_filter"]:
                search = RecipeSearch(
                    request, filter=serializer.initial_data["custom_filter"]["id"]
                )
                recipes = search.get_queryset(
            ...
            if (
                serializer.validated_data["type"] == ImportExportBase.PDF
                and not settings.ENABLE_PDF_EXPORT
            ):
                return JsonResponse(
                    {
                        "error": _(
                            "The PDF Exporter is not enabled on this instance as it is still in an experimental state."
                        )
                    }
                )

            el = ExportLog.objects.create(
            ...
            return Response(
                ExportLogSerializer(context={"request": request}).to_representation(el),
                status=status.HTTP_200_OK,
            )

        return Response(
            {"error": True, "msg": serializer.errors},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
class FdcSearchView(APIView):
    ...
    def get(self, request, format=None):
        ...
        if query is not None:
            ...
            response = requests.get(
                f'https://api.nal.usda.gov/fdc/v1/foods/search?api_key={FDC_API_KEY}&query={query}&dataType={",".join(data_types)}'
            )
            ...
            return Response(
                FdcQuerySerializer(context={"request": request}).to_representation(
...
class ImportOpenData(APIView):
    ...
    @extend_schema(
        request=ImportOpenDataSerializer(many=False),
        responses=ImportOpenDataResponseSerializer(many=False),
    )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            data_importer = OpenDataImporter(
                ...
                use_metric=serializer.validated_data["use_metric"],
            )
            ...
            return Response(
                ImportOpenDataResponseSerializer(
                    context={"request": request}
                ).to_representation(response_obj)
            )
...
class ServerSettingsViewSet(viewsets.GenericViewSet):
    permission_classes = (
        []
    )  # public view to use by anonymous request (recipe share page)
    ...
    def current(self, request, *args, **kwargs):
        ...
        s["unauthenticated_theme_from_space"] = (
            settings.UNAUTHENTICATED_THEME_FROM_SPACE
        )
        ...
        if (
            ...
            and settings.UNAUTHENTICATED_THEME_FROM_SPACE > 0
        ):
            with scopes_disabled():
                space = Space.objects.filter(
                    id=settings.UNAUTHENTICATED_THEME_FROM_SPACE
                ).first()
        ...
        if space:
            s["logo_color_32"] = (
                space.logo_color_32.file if space.logo_color_32 else None
            )
            s["logo_color_128"] = (
                space.logo_color_128.file if space.logo_color_128 else None
            )
            s["logo_color_144"] = (
                space.logo_color_144.file if space.logo_color_144 else None
            )
            s["logo_color_180"] = (
                space.logo_color_180.file if space.logo_color_180 else None
            )
            s["logo_color_192"] = (
                space.logo_color_192.file if space.logo_color_192 else None
            )
            s["logo_color_512"] = (
                space.logo_color_512.file if space.logo_color_512 else None
            )
            s["logo_color_svg"] = (
                space.logo_color_svg.file if space.logo_color_svg else None
            )
            s["custom_theme"] = (
                space.custom_space_theme.file if space.custom_space_theme else None
            )
...
def get_recipe_file(request, pk):
    recipe = get_object_or_404(
        Recipe, pk=pk
    )  # space check handled by CustomRecipePermission
    if recipe.storage:
        return FileResponse(
            get_recipe_provider(recipe).get_file(recipe), filename=f"{recipe.name}.pdf"
        )
    else:
        return FileResponse()
...
@extend_schema(
    request=inline_serializer(
        name="ShareLinkSerializer", fields={"pk": IntegerField()}
    ),
    responses=inline_serializer(
        name="ShareLinkSerializer",
        ...
        return JsonResponse(
            {
                ...
                "link": request.build_absolute_uri(
                    reverse("index") + f"recipe/{pk}/?share={link.uuid}"
                ),
            }
        )
    else:
...
@extend_schema(
    request=inline_serializer(
        name="IngredientStringSerializer", fields={"text": CharField()}
    ),
    responses=inline_serializer(
        name="ParsedIngredientSerializer",
        fields={
            "amount": IntegerField(),
            "unit": CharField(),
            "food": CharField(),
...
def ingredient_from_string(request):
    ...
    if food:
        if (
            food_obj := Food.objects.filter(space=request.space)
            .filter(Q(name=food) | Q(plural_name=food))
            .first()
        ):
    ...
    if unit:
        if (
            unit_obj := Unit.objects.filter(space=request.space)
            .filter(Q(name=unit) | Q(plural_name=unit))
            .first()
        ):

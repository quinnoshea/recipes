...
def setup_bot(request, pk):
    ...
    create_response = requests.get(
        f"https://api.telegram.org/bot{bot.token}/setWebhook?url={hook_url}"
    )
    ...
    return JsonResponse(
        {
            "hook_url": hook_url,
            "create_response": json.loads(create_response.content.decode()),
            "info_response": json.loads(info_response.content.decode()),
        },
...
def remove_bot(request, pk):
    ...
    return JsonResponse(
        {
            "remove_response": json.loads(remove_response.content.decode()),
            "info_response": json.loads(info_response.content.decode()),
        },
        json_dumps_params={"indent": 4},
    )

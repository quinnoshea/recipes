...
def index(request, path=None, resource=None):
    ...
    if (
        ...
        or re.search(r"/recipe/\d+/", request.path[:512])
        and request.GET.get("share")
    ):
...
def redirect_recipe_view(request, pk):
    ...
    return HttpResponseRedirect(
        build_absolute_uri(request, reverse("index")) + f"recipe/{pk}"
    )
...
def redirect_recipe_share_view(request, pk, share):
    if re.match(
        ...
        return HttpResponseRedirect(
            build_absolute_uri(request, reverse("index"))
            + f"recipe/{pk}/?share={share}"
        )
    ...
    if request.POST:
        ...
        if settings.HOSTED and request.user.username == "demo":
            messages.add_message(
                request,
                messages.WARNING,
                _("This feature is not available in the demo version!"),
            )
        else:
            if create_form.is_valid():
                if (
                    Space.objects.filter(created_by=request.user).count()
                    >= request.user.userpreference.max_owned_spaces
                ):
                    messages.add_message(
                        request,
                        messages.ERROR,
                        ...
                        + f" ({request.user.userpreference.max_owned_spaces})",
                    )
                ...
                messages.add_message(
                    ...
                    messages.SUCCESS,
                    _(
                        "You have successfully created your own recipe space. Start by adding some recipes or invite other people to join you."
                    ),
                )
                return HttpResponseRedirect(
                    reverse("view_switch_space", args=[user_space.space.pk])
                )

            if join_form.is_valid():
                return HttpResponseRedirect(
                    reverse("view_invite", args=[join_form.cleaned_data["token"]])
                )
    else:
        if (
            settings.SOCIAL_DEFAULT_ACCESS
            and len(request.user.userspace_set.all()) == 0
        ):
            user_space = UserSpace.objects.create(
        ...
        if "signup_token" in request.session:
            return HttpResponseRedirect(
                reverse("view_invite", args=[request.session.pop("signup_token", "")])
            )
...
def no_perm(request):
    if not request.user.is_authenticated:
        ...
        return HttpResponseRedirect(
            reverse("account_login") + "?next=" + request.GET.get("next", "/search/")
        )
...
def recipe_pdf_viewer(request, pk):
    with scopes_disabled():
        ...
        if share_link_valid(recipe, request.GET.get("share", None)) or (
            has_group_permission(request.user, ["guest"])
            and recipe.space == request.space
        ):
            return render(
                request,
                "pdf_viewer.html",
                {"recipe_id": pk, "share": request.GET.get("share", None)},
            )
        return HttpResponseRedirect(reverse("index"))
...
def system(request):
    ...
    if postgres:
        postgres_current = (
            16  # will need to be updated as PostgreSQL releases new major versions
        )
        ...
        try:
            ...
            if postgres_ver >= postgres_current:
                ...
                database_message = _(
                    "PostgreSQL %(v)s is deprecated.  Upgrade to a fully supported version!"
                ) % {"v": postgres_ver}
            else:
                database_status = "info"
                database_message = _(
                    "You are running PostgreSQL %(v1)s.  PostgreSQL %(v2)s is recommended"
    ...
    for key in migration_info.keys():
        migration_info[key]["total"] = len(
            migration_info[key]["unapplied_migrations"]
    ...
    if settings.REDIS_HOST:
        ...
        for i in range(0, 6):
            ...
            total_stats.append(
                int(r.get(f"api:request-count:{d}"))
                if r.get(f"api:request-count:{d}")
                else 0
            )
        ...
        for x in r.zrange(
            "api:endpoint-request-count", 0, -1, withscores=True, desc=True
        ):
            ...
            for i in range(0, 6):
                ...
                endpoint_stats.append(
                    r.zscore(f"api:endpoint-request-count:{d}", endpoint)
                )
    ...
    if not cache_response:
        caches["default"].set("system_view_test_cache_entry", datetime.now(), 10)
...
def plugin_update(request):
    ...
    for p in PLUGINS:
        if p["module"] == request.GET["module"]:
            update_response = subprocess.check_output(
                ["git", "pull"], cwd=p["base_path"]
            )
...
def setup(request):
    with scopes_disabled():
        if (
            ...
            or "django.contrib.auth.backends.RemoteUserBackend"
...
def invite_link(request, token):
    with scopes_disabled():
        ...
        if link := InviteLink.objects.filter(
            ...
            if (
                request.user.is_authenticated
                and not request.user.userspace_set.filter(space=link.space).exists()
            ):
                ...
                messages.add_message(
                    request, messages.SUCCESS, _("Successfully joined space.")
                )
                return HttpResponseRedirect(reverse("view_space_overview"))
            else:
...
def report_share_abuse(request, token):
    if not settings.SHARING_ABUSE:
        messages.add_message(
            ...
            messages.WARNING,
            _(
                "Reporting share links is not enabled for this instance. Please notify the page administrator to report problems."
            ),
        )
    else:
        if link := ShareLink.objects.filter(uuid=token).first():
            ...
            messages.add_message(
                ...
                messages.WARNING,
                _(
                    "Recipe sharing link has been disabled! For additional information please contact the page administrator."
                ),
            )
    return HttpResponseRedirect(reverse("index"))
...
def web_manifest(request):
    ...
    manifest_info = {
        ...
        "shortcuts": [
            {
                ...
                "url": "./mealplan",
                "icons": [
                    {"src": static("assets/logo_color_plan.svg"), "sizes": "any"},
                    {
            ...
            },
            {
                ...
                "url": "./shopping",
                "icons": [
                    {"src": static("assets/logo_color_shopping.svg"), "sizes": "any"},
                    {
        ...
        "share_target": {
            ...
            "params": {"title": "title", "url": "url", "text": "text"},
        },
    }
...
def service_worker(request):
    with open(
        os.path.join(BASE_DIR, "cookbook", "static", "vue3", "service-worker.js"), "rb"
    ...
    def find_orphans():
        ...
        for root, dirs, files in os.walk(media_dir):
            for file in files:
                ...
                if not file.lower().endswith((".db")) and not root.lower().endswith(
                    ("@eadir")
                ):

...
def sle_2(request):
    ...
    return ShoppingListEntryFactory.create_batch(
        count, space=user.userspace_set.filter(active=1).first().space, created_by=user
    )
...
def test_sharing(request, shared, count, sle_2, sle, u1_s1):
    ...
    assert json.loads(u1_s1.get(reverse(LIST_URL)).content)["count"] == 10
    assert json.loads(shared_client.get(reverse(LIST_URL)).content)["count"] == 10
...
def test_recent(sle, u1_s1):
    ...
    user.userpreference.shopping_recent_days = (
        user.userpreference.shopping_recent_days + 4
    )
    user.userpreference.save()

...
def test_list_permission(
    arg, request, space_1, g1_s1, u1_s1, a1_s1, ids=lambda arg: arg[0]
):
    space_1.created_by = auth.get_user(a1_s1)
    space_1.save()
    InviteLink.objects.create(
        group_id=1, created_by=auth.get_user(a1_s1), space=space_1
    )
...
def test_update(arg, request, space_1, u1_s1, a1_s1, ids=lambda arg: arg[0]):
    with scopes_disabled():
        ...
        il = InviteLink.objects.create(
            group_id=1, created_by=auth.get_user(a1_s1), space=space_1
        )

        c = request.getfixturevalue(arg[0])
        r = c.patch(
            reverse(DETAIL_URL, args={il.id}),
            {"email": "test@mail.de"},
...
def test_delete(u1_s1, u1_s2, a1_s1, a2_s1, space_1):
    with scopes_disabled():
        ...
        # user cant delete
        r = u1_s1.delete(reverse(DETAIL_URL, args={il.id}))
        assert r.status_code == 403

        # admin cant delete
        r = a2_s1.delete(reverse(DETAIL_URL, args={il.id}))
        assert r.status_code == 403

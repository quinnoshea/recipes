...
def test_shopping_forbidden_methods(food, u1_s1):
    r = u1_s1.post(reverse(SHOPPING_FOOD_URL, args={food.id}))
    assert r.status_code == 405

    r = u1_s1.delete(reverse(SHOPPING_FOOD_URL, args={food.id}))
    assert r.status_code == 405

    r = u1_s1.get(reverse(SHOPPING_FOOD_URL, args={food.id}))
    assert r.status_code == 405
...
def test_shopping_food_create(request, arg, food, ids=lambda arg: arg[0]):
    ...
    if r.status_code == 204:
        assert (
            len(json.loads(c.get(reverse(SHOPPING_LIST_URL)).content)["results"]) == 1
        )
...
def test_shopping_food_delete(request, arg, food, ids=lambda arg: arg[0]):
    ...
    if r.status_code == 204:
        assert (
            len(json.loads(c.get(reverse(SHOPPING_LIST_URL)).content)["results"]) == 0
        )
...
def test_shopping_food_share(u1_s1, u2_s1, food, space_1):
    ...
    assert (
        len(json.loads(u1_s1.get(reverse(SHOPPING_LIST_URL)).content)["results"]) == 1
    )
    assert (
        len(json.loads(u2_s1.get(reverse(SHOPPING_LIST_URL)).content)["results"]) == 2
    )

...
def test_update(arg, request, recipe_1_s1):
    with scopes_disabled():
        ...
        r = c.patch(
            reverse(DETAIL_URL, args={recipe_1_s1.id}),
            j,
...
def test_update_share(u1_s1, u2_s1, u1_s2, recipe_1_s1):
    ...
    r = u1_s1.patch(
        reverse(DETAIL_URL, args={recipe_1_s1.id}),
        {
            "shared": [
                {
                    "id": auth.get_user(u1_s2).pk,
                    "username": auth.get_user(u1_s2).username,
                },
                {
                    "id": auth.get_user(u2_s1).pk,
                    "username": auth.get_user(u2_s1).username,
                },
            ]
        },
        content_type="application/json",

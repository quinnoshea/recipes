...
def obj_1(space_1, u1_s1, recipe_1_s1):
    return CookLog.objects.create(
        recipe=recipe_1_s1, created_by=auth.get_user(u1_s1), space=space_1
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            "g1_s1",
            403,
        ],  # changed expected value.  based on list permissions the log is visible, but not editable
        ["u1_s1", 200],
        [
            "a1_s1",
            403,
...
def test_add(arg, request, u1_s2, u2_s1, recipe_1_s1):
    c = request.getfixturevalue(arg[0])
    r = c.post(
        reverse(LIST_URL), {"recipe": recipe_1_s1.id}, content_type="application/json"
    )
    assert r.status_code == arg[1]
    if r.status_code == 201:
        ...
        assert (
            r.status_code == 403
        )  # expected value changed.  user can list the log - detail should be 403 as no reason to 'hide' that it actually exists

...
def test_update(arg, request, space_1, u1_s1, a1_s1):
    with scopes_disabled():
        ...
        c = request.getfixturevalue(arg[0])
        r = c.patch(
            reverse(DETAIL_URL, args={user_space.id}),
...
def test_update_space_owner(a1_s1, space_1):
    ...
    space_1.save()
    r = a1_s1.patch(
        reverse(DETAIL_URL, args={auth.get_user(a1_s1).userspace_set.first().id}),
...
def test_delete(u1_s1, u1_s2, a1_s1, space_1):
    ...
    r = u1_s1.delete(
        reverse(DETAIL_URL, args={auth.get_user(u1_s1).userspace_set.first().id})
    )

...
def test_list_space(recipe_1_s1, u1_s1, u1_s2, space_2):
    ...
    with scopes_disabled():
        ...
        Step.objects.update(
            space=Subquery(
                Step.objects.filter(pk=OuterRef("pk")).values("recipe__space")[:1]
            )
        )
        Ingredient.objects.update(
            space=Subquery(
                Ingredient.objects.filter(pk=OuterRef("pk")).values(
                    "step__recipe__space"
                )[:1]
            )
        )
...
def test_add(arg, request, u1_s2):
    ...
    if r.status_code == 201:
        ...
        assert (
            r.status_code == 404
        )  # ingredient is not linked to a recipe and therefore cannot be accessed
...
def test_delete(u1_s1, u1_s2, recipe_1_s1):
    with scopes_disabled():
        s = recipe_1_s1.steps.first()
        r = u1_s2.delete(reverse(DETAIL_URL, args={s.id}))
        assert r.status_code == 404

        r = u1_s1.delete(reverse(DETAIL_URL, args={s.id}))

...
def meal_type(space_1, u1_s1):
    return MealType.objects.get_or_create(
        name="test", space=space_1, created_by=auth.get_user(u1_s1)
    )[0]
...
def test_add_with_shopping(u1_s1, meal_type):
    ...
    assert (
        len(
            json.loads(u1_s1.get(reverse("api:shoppinglistentry-list")).content)[
                "results"
            ]
        )
        == 10
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [f'?to_date={(timezone.now() - timedelta(days=1)).strftime("%Y-%m-%d")}', 1],
        [

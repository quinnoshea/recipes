...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "steps__food_recipe_count": {"step": 0, "count": 1},
...
def test_shopping_recipe_method(request, arg, recipe, sle_count, u1_s1, u2_s1):
    ...
    if r.status_code == 204:  # skip anonymous user
        ...
        assert (
            json.loads(u2_s1.get(reverse(SHOPPING_LIST_URL)).content)["count"]
            == sle_count
        )
        # confirm that the author of the recipe doesn't have access to shopping list
        if c != u1_s1:
            assert (
                json.loads(u1_s1.get(reverse(SHOPPING_LIST_URL)).content)["count"] == 0
            )
...
def test_shopping_recipe_mixed_authors(u1_s1, u2_s1, space_1):
    with scopes_disabled():
        ...
        food.save()
        recipe1.steps.add(
            StepFactory(step_recipe=recipe3, ingredients__count=0, space=space)
        )
        recipe1.save()
...
@pytest.mark.parametrize(
    "recipe", [{"steps__ingredients__header": 1}], indirect=["recipe"]
)

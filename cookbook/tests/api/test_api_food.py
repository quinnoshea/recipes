        ...
        for i in inherit:
            Through.objects.bulk_create(
                [
                    ...
                    for x in Food.objects.filter(space=space_1).values_list(
                        "id", flat=True
                    )
                ]
            )

    Food.objects.get(id=child_id).move(Food.objects.get(id=obj_id), node_location)

    Food.objects.get(id=obj_id).move(Food.objects.get(id=parent_id), node_location)
...
def test_add_duplicate(u1_s1, u1_s2, obj_1, obj_3):
    ...
    assert json.loads(u1_s2.get(reverse(LIST_URL)).content)["count"] == 1
    r = u1_s1.post(
        reverse(LIST_URL), {"name": obj_1.name}, content_type="application/json"
    )
    response = json.loads(r.content)
...
def test_integrity(u1_s1, recipe_1_s1):
    ...
    r = u1_s1.delete(reverse(DETAIL_URL, args={f_1.id}))
...
def test_merge_errors(u1_s1, obj_tree_1, obj_3, space_1):
    ...
    r = u1_s1.put(reverse(MERGE_URL, args=[obj_tree_1.id, 9999]))
    assert r.status_code == 404

    # attempt to merge non-existent node to parent
    r = u1_s1.put(reverse(MERGE_URL, args=[9999, obj_tree_1.id]))
    assert r.status_code == 404
    # attempt to move to wrong space
    r = u1_s1.put(reverse(MERGE_URL, args=[obj_tree_1.id, obj_3.id]))
    assert r.status_code == 404

    # attempt to merge with child
    r = u1_s1.put(reverse(MERGE_URL, args=[parent.id, obj_tree_1.id]))
    assert r.status_code == 403

    # attempt to merge with self
    r = u1_s1.put(reverse(MERGE_URL, args=[obj_tree_1.id, obj_tree_1.id]))
    assert r.status_code == 403
...
def test_root_filter(obj_tree_1, obj_2, obj_3, u1_s1):
    ...
    response = json.loads(
        u1_s1.get(
            f"{reverse(LIST_URL)}?root={parent.id}&query={obj_2.name[4:]}"
        ).content
    )
    assert response["count"] == 2
...
def test_tree_filter(obj_tree_1, obj_2, obj_3, u1_s1):
    ...
    response = json.loads(
        u1_s1.get(
            f"{reverse(LIST_URL)}?tree={parent.id}&query={obj_2.name[4:]}"
        ).content
    )
    assert response["count"] == 4
...
@pytest.mark.parametrize(
    "obj_tree_1, field, inherit, new_val",
    [
        (
    ...
    indirect=["obj_tree_1"],
)  # indirect=True populates magic variable request.param of obj_tree_1 with the parameter
...
def test_onhand(obj_1, u1_s1, u2_s1, space_1):
    assert (
        json.loads(u1_s1.get(reverse(DETAIL_URL, args={obj_1.id})).content)[
            "food_onhand"
        ]
        is False
    )
    assert (
        json.loads(u2_s1.get(reverse(DETAIL_URL, args={obj_1.id})).content)[
            "food_onhand"
        ]
        is False
    )

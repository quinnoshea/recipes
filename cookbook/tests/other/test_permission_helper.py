...
def test_has_group_permission(u1_s1, a_u, space_2):
    with scopes_disabled():
        ...
        us = UserSpace.objects.create(
            user=auth.get_user(u1_s1), space=space_2, active=False
        )
...
def test_is_owner(u1_s1, u2_s1, u1_s2, a_u, space_1, recipe_1_s1):
    with scopes_disabled():
        ...
        rb = RecipeBook.objects.create(
            name="Test", created_by=auth.get_user(u1_s1), space=space_1
        )
...
def test_switch_user_active_space(u1_s1, u1_s2, space_1, space_2):
    with scopes_disabled():
        ...
        assert (
            switch_user_active_space(auth.get_user(u1_s1), space_1)
            == auth.get_user(u1_s1).userspace_set.filter(active=True).get()
        )

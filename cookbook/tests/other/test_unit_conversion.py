...
from cookbook.models import Food, Ingredient, Unit, UnitConversion
...
def test_base_converter(space_1):
    ...
    assert (
        abs(
            uch.convert_from_to("imperial_gallon", "gallon", 1000) - Decimal(1200.95104)
        )
        < 0.00001
    )
...
def test_unit_conversions(space_1, space_2, u1_s1):
    with scopes_disabled():
        ...
        assert (
            abs(next(x for x in conversions if x.unit == unit_kg).amount - Decimal(0.1))
            < 0.0001
        )
        ...
        assert (
            abs(
                next(x for x in conversions if x.unit == unit_floz2).amount
                - Decimal(96.07599404038842)
            )
            < 0.001
        )  # TODO validate value
        ...
        assert (
            abs(
                next(x for x in conversions if x.unit == unit_pint).amount
                - Decimal(6.004749627524276)
            )
            < 0.001
        )  # TODO validate value
        ...
        assert (
            abs(
                next(x for x in conversions if x.unit == unit_fantasy).amount
                - Decimal("133.700")
            )
            < 0.001
        )  # TODO validate value
        ...
        assert (
            abs(
                next(x for x in conversions if x.unit == unit_gram).amount
                - Decimal(1000)
            )
            < 0.0001
        )
        assert (
            abs(next(x for x in conversions if x.unit == unit_kg).amount - Decimal(1))
            < 0.0001
        )
        print(conversions)
        ...
        assert (
            abs(
                next(x for x in conversions if x.unit == unit_kg_space_2).amount
                - Decimal(0.1)
            )
            < 0.0001
        )

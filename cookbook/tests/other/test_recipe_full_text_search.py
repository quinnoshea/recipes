...
def found_recipe(request, space_1, accent, unaccent, u1_s1, u2_s1):
    ...
    if request.param.get("food", None):
        ...
        recipe3.steps.first().ingredients.add(
            IngredientFactory.create(food=obj1), IngredientFactory.create(food=obj2)
        )
    ...
    if request.param.get("unit", None):
        ...
        recipe3.steps.first().ingredients.add(
            IngredientFactory.create(unit=obj1), IngredientFactory.create(unit=obj2)
        )
    ...
    if request.param.get("viewedon", None):
        ViewLogFactory.create(
            recipe=recipe1, created_by=user1, created_at=days_3, space=space_1
        )
        ViewLogFactory.create(
            recipe=recipe2, created_by=user1, created_at=days_30, space=space_1
        )
        ViewLogFactory.create(
            recipe=recipe3, created_by=user2, created_at=days_15, space=space_1
        )
...
def test_search_or_and_not(found_recipe, param_type, operator, recipes, u1_s1, space_1):
    with scope(space=space_1):
        ...
        r = json.loads(
            u1_s1.get(reverse(LIST_URL) + f"?{param1_not}&{param2_not}").content
        )
        ...
        assert (
            len(
                [
                    x["id"]
                    for x in r["results"]
                    if x["id"] in [found_recipe[3].id, found_recipe[4].id]
                ]
            )
            == user1[1]
        )
        ...
        assert (
            len(
                [
                    x["id"]
                    for x in r["results"]
                    if x["id"] in [found_recipe[0].id, found_recipe[1].id]
                ]
            )
            == user1[1]
        )

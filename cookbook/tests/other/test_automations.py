...
def test_regex_automation(u1_s1, arg, source):
    ...
    automation = AutomationEngine(
        request,
        use_cache=False,
        source="https://www.allrecipes.com/recipe/24010/easy-chicken-marsala/",
    )
    ...
    with scope(space=space):
        ...
        assert (
            automation.apply_regex_replace_automation(middle, arg[0]) == target
        ) == source[1]
        assert (
            automation.apply_regex_replace_automation(beginning, arg[0]) == target
        ) == source[1]
        assert (
            automation.apply_regex_replace_automation(fail, arg[0]) == target
        ) == False
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            "longer string second first longer string",
            "longer string first second longer string",
        ],
...
def test_url_import_regex_replace(u1_s1):
    ...
    if "cookbook" in os.getcwd():
        ...
        test_file = os.path.join(
            os.getcwd(), "cookbook", "tests", "other", "test_data", recipe
        )
    with open(test_file, "r", encoding="UTF-8") as d:
        scrape = scrape_html(
            html=d.read(), org_url="https://testrecipe.test", supported_only=False
        )
    with scope(space=space):
        for t in types:
            Automation.objects.get_or_create(

...
def shopping_helper(qs, request):
    ...
    user = request.user
    supermarket_order = [
        F("food__supermarket_category__name").asc(nulls_first=True),
        "food__name",
    ]
    ...
    if supermarket:
        supermarket_categories = SupermarketCategoryRelation.objects.filter(
            supermarket=supermarket, category=OuterRef("food__supermarket_category")
        )
        qs = qs.annotate(
            supermarket_order=Coalesce(
                Subquery(supermarket_categories.values("order")), Value(9999)
            )
        )
...
class RecipeShoppingEditor:
    def __init__(self, user, space, **kwargs):
        ...
        if isinstance(self.mealplan, dict):
            self.mealplan = MealPlan.objects.filter(
                id=self.mealplan["id"], space=self.space
            ).first()
        self.id = self._kwargs.get("id", None)

        self._shopping_list_recipe = self.get_shopping_list_recipe(
            self.id, self.created_by, self.space
        )
        ...
        if self._shopping_list_recipe:
            ...
            self.created_by = getattr(
                self._shopping_list_recipe.entries.first(),
                "created_by",
                self.created_by,
            )

        self.recipe = (
            getattr(self._shopping_list_recipe, "recipe", None)
            or self._kwargs.get("recipe", None)
            or getattr(self.mealplan, "recipe", None)
        )
        ...
        try:
            ...
            self.servings = (
                getattr(self._shopping_list_recipe, "servings", None)
                or getattr(self.mealplan, "servings", None)
                or getattr(self.recipe, "servings", None)
            )

    @property
    def _recipe_servings(self):
        return (
            getattr(self.recipe, "servings", None)
            or getattr(getattr(self.mealplan, "recipe", None), "servings", None)
            or getattr(
                getattr(self._shopping_list_recipe, "recipe", None), "servings", None
            )
        )
    ...
    def get_shopping_list_recipe(id, user, space):
        return (
            ShoppingListRecipe.objects.filter(id=id)
            .filter(entries__space=space)
            .filter(
    ...
    def get_recipe_ingredients(self, id, exclude_onhand=False):
        if exclude_onhand:
            return Ingredient.objects.filter(
                step__recipe__id=id, food__ignore_shopping=False, space=self.space
            ).exclude(food__onhand_users__id__in=[x.id for x in self._shared_users])
        else:
            return Ingredient.objects.filter(
                step__recipe__id=id, food__ignore_shopping=False, space=self.space
            )
    ...
    def create(self, **kwargs):
        ...
        if mealplan := kwargs.get("mealplan", None):
            if isinstance(mealplan, dict):
                self.mealplan = MealPlan.objects.filter(
                    id=mealplan["id"], space=self.space
                ).first()
            else:
        ...
        if not self.servings:
            self.servings = getattr(self.mealplan, "servings", None) or getattr(
                self.recipe, "servings", 1.0
            )

        self._shopping_list_recipe = ShoppingListRecipe.objects.create(
            recipe=self.recipe,
            ...
            if self._include_related:
                ...
                self._add_ingredients(
                    self.get_recipe_ingredients(
                        self.recipe.id, exclude_onhand=exclude_onhand
                    ).exclude(food__recipe__in=related)
                )
                for r in related:
                    self._add_ingredients(
                        self.get_recipe_ingredients(
                            r.id, exclude_onhand=exclude_onhand
                        ).exclude(food__recipe__in=related)
                    )
            else:
                self._add_ingredients(
    ...
    def edit_servings(self, servings=None, **kwargs):
        ...
        for sle in ShoppingListEntry.objects.filter(
            list_recipe=self._shopping_list_recipe
        ):
            if (
                sle.ingredient
            ):  # TODO temporarily dont scale manual entries until ingredient_amount or some other base amount has been migrated to SLE
    ...
    def _add_ingredients(self, ingredients=None):
        if not ingredients:
            ...
            ingredients = Ingredient.objects.filter(
                id__in=ingredients, food__ignore_shopping=False
            )
        existing = self._shopping_list_recipe.entries.filter(
            ingredient__in=ingredients
        ).values_list("ingredient__pk", flat=True)
    ...
    def _delete_ingredients(self, ingredients=None):
        if not ingredients:
            return
        to_delete = self._shopping_list_recipe.entries.exclude(
            ingredient__in=ingredients
        )

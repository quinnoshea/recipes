...
class ScopeMiddleware:
    ...
    def __call__(self, request):
        ...
        if request.user.is_authenticated:
            ...
            if request.path.startswith(prefix + "/signup/") or request.path.startswith(
                prefix + "/invite/"
            ):
                return self.get_response(request)
            ...
            with scopes_disabled():
                if (
                    request.user.userspace_set.count() == 0
                    and not reverse("account_logout") in request.path
                ):

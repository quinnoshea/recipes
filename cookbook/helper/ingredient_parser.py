...
class IngredientParser:
        ...
        if len(unit) > 0:
            if self.ignore_rules:
                u, created = Unit.objects.get_or_create(
                    name=unit.strip(), space=self.request.space
                )
            else:
                u, created = Unit.objects.get_or_create(
                    name=self.automation.apply_unit_automation(unit),
        ...
        if len(food) > 0:
            if self.ignore_rules:
                f, created = Food.objects.get_or_create(
                    name=food.strip(), space=self.request.space
                )
            else:
                f, created = Food.objects.get_or_create(
                    name=self.automation.apply_food_automation(food),
                    space=self.request.space,
                )
            return f
        return None
    ...
    def parse_fraction(self, x):
        if len(x) == 1 and "fraction" in unicodedata.decomposition(x):
            ...
            return float((frac_split[1]).replace("003", "")) / float(
                (frac_split[3]).replace("003", "")
            )
        else:
            frac_split = x.split("/")
    ...
    def parse(self, ingredient):
        ...
        if len(ingredient) < 1000 and re.search(
            r"^([^\W\d_])+(.)*[1-9](\d)*\s*([^\W\d_])+", ingredient
        ):
            match = re.search(r"[1-9](\d)*\s*([^\W\d_])+", ingredient)
            ingredient = (
                ingredient[match.start() : match.end()]
                + " "
                + ingredient.replace(ingredient[match.start() : match.end()], "")
            )
        ...
        if re.match("(.){1,6}\\s\\((.[^\\(\\)])+\\)\\s", ingredient):
            ...
            ingredient = (
                ingredient[: match.start()]
                + ingredient[match.end() :]
                + " "
                + ingredient[match.start() : match.end()]
            )
        ...
        if len(tokens) == 1:
            ...
            try:
                ...
                if len(tokens) > 2:
                    ...
                    if not self.ignore_rules:
                        tokens, never_unit_applied = (
                            self.automation.apply_never_unit_automation(tokens)
                        )
        ...
        if (
            len(food) > Food._meta.get_field("name").max_length
        ):  # test if food name is to long
            # try splitting it at a space and taking only the first arg
            if (
                len(food.split()) > 1
                and len(food.split()[0]) < Food._meta.get_field("name").max_length
            ):
        ...
        return (
            ...
            unit,
            food,
            note[: Ingredient._meta.get_field("note").max_length].strip(),
        )

...
class OpenDataImporter:
    ...
    def _update_slug_cache(self, object_class, datatype):
        self.slug_id_cache[datatype] = dict(
            object_class.objects.filter(
                space=self.request.space, open_data_slug__isnull=False
            ).values_list(
                "open_data_slug",
                "id",
            )
    ...
    def _is_obj_identical(field_list, obj, existing_obj):
        ...
        for field in field_list:
            if isinstance(getattr(obj, field), float) or isinstance(
                getattr(obj, field), Decimal
            ):
                if (
                    abs(float(getattr(obj, field)) - float(existing_obj[field])) > 0.001
                ):  # convert both to float and check if basically equal
                    print(
                        f"comparing FLOAT {obj} failed because field {field} is not equal ({getattr(obj, field)} != {existing_obj[field]})"
                    )
                    return False
            elif getattr(obj, field) != existing_obj[field]:
                print(
                    f"comparing {obj} failed because field {field} is not equal ({getattr(obj, field)} != {existing_obj[field]})"
                )
                return False
        return True

    @staticmethod
    def _merge_if_conflicting(
        model_type, obj, existing_data_slugs, existing_data_names
    ):
        """
        ...
        """
        if (
            obj.open_data_slug in existing_data_slugs
            and obj.name in existing_data_names
            and existing_data_slugs[obj.open_data_slug]["pk"]
            != existing_data_names[obj.name]["pk"]
        ):
            try:
                source_obj = model_type.objects.get(
                    pk=existing_data_slugs[obj.open_data_slug]["pk"]
                )
                del existing_data_slugs[obj.open_data_slug]
                source_obj.merge_into(
                    model_type.objects.get(pk=existing_data_names[obj.name]["pk"])
                )
                return True
            except RuntimeError:
    ...
    def import_units(self):
        ...
        existing_data_slugs = {}
        existing_data_names = {}
        for obj in model_type.objects.filter(space=self.request.space).values(
            "pk", *field_list
        ):
        ...
        for u in list(self.data[datatype].keys()):
            obj = model_type(
                ...
                base_unit=(
                    self.data[datatype][u]["base_unit"].lower()
                    if self.data[datatype][u]["base_unit"] != ""
                    else None
                ),
                open_data_slug=u,
            ...
            if (
                obj.open_data_slug in existing_data_slugs
                or obj.name in existing_data_names
            ):
                if not self._merge_if_conflicting(
                    model_type, obj, existing_data_slugs, existing_data_names
                ):
                    od_response.total_errored += 1
                    continue  # if conflicting objects exist and cannot be merged skip object

                existing_obj = self._get_existing_obj(
                    obj, existing_data_slugs, existing_data_names
                )
    ...
    def import_supermarket(self):
        ...
        for k in list(self.data[datatype].keys()):
            ...
            SupermarketCategoryRelation.objects.bulk_create(
                relations,
    ...
    def import_food(self):
        ...
        for k in list(self.data[datatype].keys()):
            ...
            obj_dict = {
                "name": self.data[datatype][k]["name"],
                "plural_name": (
                    self.data[datatype][k]["plural_name"]
                    if self.data[datatype][k]["plural_name"] != ""
                    else None
                ),
                "supermarket_category_id": (
                    self.slug_id_cache["category"][
                        self.data[datatype][k]["store_category"]
                    ]
                    if self.data[datatype][k]["store_category"]
                    in self.slug_id_cache["category"]
                    else None
                ),
                "fdc_id": (
                    re.sub(r"\D", "", self.data[datatype][k]["fdc_id"])
                    if self.data[datatype][k]["fdc_id"] != ""
                    else None
                ),
                "open_data_slug": k,
        ...
        for k in list(self.data["food"].keys()):
            for fp in self.data["food"][k]["properties"]["type_values"]:
                obj = model_type(
                    property_type_id=self.slug_id_cache["property"][
                        fp["property_type"]
                    ],
                ...
                if (
                    obj.open_data_food_slug in existing_data_slugs
                    and obj.property_type_id in existing_data_property_types
                    and existing_data_slugs[obj.open_data_food_slug]
                    == existing_data_property_types[obj.property_type_id]
                ):
        ...
        linked_properties = list(
            FoodProperty.objects.filter(food__space=self.request.space)
            .values_list("property_id", flat=True)
            .all()
        )

        property_food_relation_list = []
        for p in model_type.objects.filter(
            space=self.request.space, open_data_food_slug__isnull=False
        ).values_list(
            "open_data_food_slug",
            "id",
        ):
            ...
            if p[0] in self.slug_id_cache["food"] and p[1] not in linked_properties:
                property_food_relation_list.append(
                    Food.properties.through(
                        food_id=self.slug_id_cache["food"][p[0]], property_id=p[1]
                    )
                )
        ...
        for k in list(self.data[datatype].keys()):
            ...
            try:
                obj = model_type(
                    ...
                    base_unit_id=self.slug_id_cache["unit"][
                        self.data[datatype][k]["base_unit"]
                    ],
                    converted_amount=Decimal(
                        self.data[datatype][k]["converted_amount"]
                    ),
                    converted_unit_id=self.slug_id_cache["unit"][
                        self.data[datatype][k]["converted_unit"]
                    ],
                ...
                if obj.open_data_slug in existing_data_slugs:
                    ...
                    if obj.food_id in existing_data_foods:
                        for edf in existing_data_foods[obj.food_id]:
                            if (
                                obj.base_unit_id == edf["base_unit_id"]
                                and obj.converted_unit_id == edf["converted_unit_id"]
                            ):
                ...
                od_response.total_errored += 1
                print(
                    self.data[datatype][k]["food"]
                    + ' is not in self.slug_id_cache["food"]'
                )

        if self.update_existing and len(update_list) > 0:
            od_response.total_updated = model_type.objects.bulk_update(
                update_list, field_list
            )
            od_response.total_errored += len(update_list) - od_response.total_updated

        if len(create_list) > 0:
            objs_created = model_type.objects.bulk_create(
                create_list,
                ignore_conflicts=True,
                unique_fields=(
                    "space",

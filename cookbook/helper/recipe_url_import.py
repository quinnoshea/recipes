...
def get_from_scraper(scrape, request):
    ...
    recipe_json["name"] = automation_engine.apply_regex_replace_automation(
        recipe_json["name"], Automation.NAME_REPLACE
    )
    ...
    recipe_json["description"] = automation_engine.apply_regex_replace_automation(
        recipe_json["description"], Automation.DESCRIPTION_REPLACE
    )
    ...
    try:
        ...
        try:
            recipe_json["working_time"] = (
                get_minutes(scrape.schema.data.get("prepTime")) or 0
            )
        except Exception:
    ...
    try:
        ...
        try:
            recipe_json["waiting_time"] = (
                get_minutes(scrape.schema.data.get("cookTime")) or 0
            )
        except Exception:
    ...
    if not recipe_json["image_url"]:
        try:
            recipe_json["image_url"] = (
                parse_image(scrape.schema.data.get("image")) or ""
            )
        except Exception:
            recipe_json["image_url"] = ""
    ...
    try:
        recipe_json["keywords"] = parse_keywords(
            list(set(map(str.casefold, keywords))), request
        )
    ...
    if (
        len(recipe_json["description"]) > 256
    ):  # split at 256 as long descriptions don't look good on recipe cards
        recipe_json["steps"][0]["instruction"] = (
            f"*{recipe_json['description']}*  \n\n"
            + recipe_json["steps"][0]["instruction"]
        )
    ...
    try:
        recipe_json["properties"] = get_recipe_properties(
            request.space, scrape.schema.nutrients()
        )
    ...
    for s in recipe_json["steps"]:
        s["instruction"] = automation_engine.apply_regex_replace_automation(
            s["instruction"], Automation.INSTRUCTION_REPLACE
        )
...
def get_recipe_properties(space, property_data):
    ...
    for pt in PropertyType.objects.filter(
        space=space, open_data_slug__in=list(properties.keys())
    ).all():
                    ...
                    recipe_properties.append(
                        {
                            ...
                            "property_amount": parse_servings(
                                property_data[properties[p]]
                            )
                            / serving_size,
                        }
                    )
...
def get_from_youtube_scraper(url, request):
    ...
    try:
        ...
        default_recipe_json["name"] = automation_engine.apply_regex_replace_automation(
            video.title, Automation.NAME_REPLACE
        )
        default_recipe_json["image"] = video.thumbnail_url
        if video.description:
            default_recipe_json["steps"][0]["instruction"] = (
                automation_engine.apply_regex_replace_automation(
                    video.description, Automation.INSTRUCTION_REPLACE
                )
            )
    except Exception:
        traceback.print_exc()
...
def clean_instruction_string(instruction):
    ...
    return (
        normalized_string.replace("", _("reverse rotation"))
                    ...
                    for ile in i["itemListElement"]:
                        if isinstance(ile, str):
                            ...
                            instruction_list.append(
                                clean_instruction_string(ile["text"])
                            )
                else:
...
def parse_keywords(keyword_json, request):
    ...
    for kw in keyword_json:
        ...
        if len(kw) != 0:
            ...
            if k := Keyword.objects.filter(
                name__iexact=kw, space=request.space
            ).first():
                keywords.append(
                    {
                        "label": str(k),
...
def iso_duration_to_minutes(string):
    ...
    return (
        int(match["days"] or 0) * 24 * 60
        + int(match["hours"] or 0) * 60
        + int(match["minutes"] or 0)
    )

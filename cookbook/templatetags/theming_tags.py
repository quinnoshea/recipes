...
from recipes.settings import FORCE_THEME_FROM_SPACE, UNAUTHENTICATED_THEME_FROM_SPACE
...
def get_theming_values(request):
    ...
    if (
        ...
        and UNAUTHENTICATED_THEME_FROM_SPACE > 0
        and FORCE_THEME_FROM_SPACE == 0
    ):
    ...
    nav_text_type_mapping = {
        Space.DARK: "navbar-light",
        Space.LIGHT: "navbar-dark",
    }  # inverted since navbar-dark means the background
    ...
    if request.user.is_authenticated:
        ...
        if (
            request.user.userpreference.nav_text_color
            and request.user.userpreference.nav_text_color in nav_text_type_mapping
        ):
            tv["nav_text_class"] = nav_text_type_mapping[
                request.user.userpreference.nav_text_color
            ]

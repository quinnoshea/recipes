...
class HomeAssistant(Connector):
    ...
    def __init__(self, config: ConnectorConfig):
        ...
        self._logger = logging.getLogger(
            f"recipes.connector.homeassistant.{config.name}"
        )
    ...
    async def homeassistant_api_call(self, method: str, path: str, data: Dict) -> str:
        ...
        async with request(
            method, urljoin(self._config.url, path), headers=headers, json=data
        ) as response:
            response.raise_for_status()
            return await response.json()

    async def on_shopping_list_entry_created(
        self, shopping_list_entry: ShoppingListEntryDTO
    ) -> None:
        ...
        self._logger.debug(
            f"adding {item=} with {description=} to {self._config.todo_entity}"
        )
        ...
        try:
            ...
            self._logger.warning(
                f"received an exception from the api: {err.request_info.url=}, {err.request_info.method=}, {err.status=}, {err.message=}, {type(err)=}"
            )

    async def on_shopping_list_entry_updated(
        self, shopping_list_entry: ShoppingListEntryDTO
    ) -> None:
    ...
    async def on_shopping_list_entry_deleted(
        self, shopping_list_entry: ShoppingListEntryDTO
    ) -> None:
...
def _format_shopping_list_entry(
    shopping_list_entry: ShoppingListEntryDTO,
) -> Tuple[str, str]:

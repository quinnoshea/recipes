...
from cookbook.helper.theme.loader import get_available_themes, get_theme_loader
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        create_parser = subparsers.add_parser(
            "create", help="Create a new theme template"
        )
    ...
    def list_themes(self, options):
        ...
        if options.get("json"):
            theme_data = {tid: theme.to_dict() for tid, theme in themes.items()}
    ...
    def create_theme(self, options):
        ...
        if not theme_id.replace("-", "").replace("_", "").isalnum():
            raise CommandError(
                "Theme ID must be alphanumeric with hyphens/underscores only"
            )

        theme_file = (
            Path(settings.BASE_DIR) / "cookbook" / "themes" / f"{theme_id}.json"
        )
        ...
        self.console.print(
            "[yellow]Edit the file to customize colors and properties[/yellow]"
        )

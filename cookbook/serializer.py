...
from recipes.settings import AWS_ENABLED, EMAIL_HOST, MEDIA_URL
        ...
        for f in list(data):
            if f not in self.fields:
                ...
                if isinstance(data[f], int):
                    ...
                    required_fields = ["id"] + [
                        field_name
                        for field_name, field in self.fields[f]
                        .__class__()
                        .fields.items()
                        if field.required
                    ]
                    data[f] = model_to_dict(
                        self.fields[f].Meta.model.objects.get(id=data[f]),
                ...
                if any(isinstance(x, int) for x in data[f]):
                    ...
                    data[f] = [x for x in data[f] if not isinstance(x, int)] + list(
                        self.fields[f]
                        .child.Meta.model.objects.filter(id__in=pk_data)
...
class ExtendedRecipeMixin(serializers.ModelSerializer):
    ...
    def get_fields(self, *args, **kwargs):
        ...
        try:
            if (
                str2bool(self.context["request"].query_params.get("extended", False))
                and self.__class__ == api_serializer
            ):
                return fields
...
class OpenDataModelMixin(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        if (
            "open_data_slug" in validated_data
            and validated_data["open_data_slug"] is not None
            and validated_data["open_data_slug"].strip() == ""
        ):
    ...
    def to_representation(self, obj):
        ...
        if c := caches["default"].get(
            ...
            try:
                shared_users = [
                    x.id
                    for x in list(self.context["request"].user.get_shopping_share())
    ...
    def to_representation(self, data):
        ...
        if self.child.Meta.model == User:
            ...
            if isinstance(self.context["request"].user, AnonymousUser):
                ...
                data = data.filter(
                    userspace__space=self.context["request"].user.get_active_space()
                ).all()
        elif isinstance(data, list):
            data = [
                d
                for d in data
                if getattr(d, self.child.Meta.model.get_space_key()[0])
                == self.context["request"].space
            ]
        else:
            data = data.filter(
                **{
                    "__".join(self.child.Meta.model.get_space_key()): self.context[
                        "request"
                    ].space
                }
            )
...
class UserFileSerializer(serializers.ModelSerializer):
    ...
    def get_download_link(self, obj):
        return self.context["request"].build_absolute_uri(
            reverse("api_download_file", args={obj.pk})
        )
        ...
        if "file" in validated_data:
            ...
            try:
                current_file_size_mb = (
                    ...
                    ).aggregate(Sum("file_size_kb"))["file_size_kb__sum"]
                    / 1000
                )
            except TypeError:
            ...
            if (
                ...
                > self.context["request"].space.max_file_storage_mb
                != 0
            ):
...
class SpaceSerializer(WritableNestedModelSerializer):
    ...
    ai_monthly_credits_used = serializers.SerializerMethodField(
    ...
    custom_space_theme = UserFileViewSerializer(
    ...
    def get_file_size_mb(self, obj):
        try:
            return (
                UserFile.objects.filter(space=obj).aggregate(Sum("file_size_kb"))[
                    "file_size_kb__sum"
                ]
                / 1000
            )
...
class UserSpaceSerializer(WritableNestedModelSerializer):
    ...
    def validate(self, data):
        if (
            self.instance.user == self.context["request"].space.created_by
        ):  # can't change space owner permission
            raise serializers.ValidationError(
                _("Cannot modify Space owner permission.")
            )
...
class UserPreferenceSerializer(WritableNestedModelSerializer):
    user = UserSerializer(read_only=True)
    food_inherit_default = serializers.SerializerMethodField(
        "get_food_inherit_defaults"
    )
    ...
    def get_food_inherit_defaults(self, obj):
        return FoodInheritFieldSerializer(
            obj.user.get_active_space().food_inherit.all(), many=True
        ).data
...
class KeywordSerializer(UniqueFieldsMixin, ExtendedRecipeMixin):
    ...
    def create(self, validated_data):
        ...
        obj, created = Keyword.objects.get_or_create(
            name=name, space=space, defaults=validated_data
        )
        return obj
...
class UnitSerializer(UniqueFieldsMixin, ExtendedRecipeMixin, OpenDataModelMixin):
    ...
    def create(self, validated_data):
        ...
        if unit := Unit.objects.filter(
            Q(name__iexact=validated_data["name"])
            | Q(plural_name__iexact=validated_data["name"]),
            space=space,
        ).first():
            return unit

        obj, created = Unit.objects.get_or_create(
            name__iexact=validated_data["name"], space=space, defaults=validated_data
        )
        return obj
...
class SupermarketCategorySerializer(
    UniqueFieldsMixin, WritableNestedModelSerializer, OpenDataModelMixin
):

    def create(self, validated_data):
        validated_data["name"] = validated_data["name"].strip()
        space = validated_data.pop("space", self.context["request"].space)
        obj, created = SupermarketCategory.objects.get_or_create(
            name__iexact=validated_data["name"], space=space, defaults=validated_data
        )
        return obj

    def update(self, instance, validated_data):
        return super(SupermarketCategorySerializer, self).update(
...
class SupermarketSerializer(
    UniqueFieldsMixin, SpacedModelSerializer, OpenDataModelMixin
):
    ...
    def create(self, validated_data):
        validated_data["name"] = validated_data["name"].strip()
        space = validated_data.pop("space", self.context["request"].space)
        obj, created = Supermarket.objects.get_or_create(
            name__iexact=validated_data["name"], space=space, defaults=validated_data
        )
        return obj
...
class PropertyTypeSerializer(
    OpenDataModelMixin, WritableNestedModelSerializer, UniqueFieldsMixin
):
...
class FoodSerializer(
    ...
    inherit_fields = FoodInheritFieldSerializer(
        many=True, allow_null=True, required=False
    )
    child_inherit_fields = FoodInheritFieldSerializer(
        many=True, allow_null=True, required=False
    )
    ...
    def create(self, validated_data):
        ...
        if (
            "supermarket_category" in validated_data
            and validated_data["supermarket_category"]
        ):
            sm_category = validated_data["supermarket_category"]
            sc_name = sm_category.pop("name", None)
            validated_data["supermarket_category"], sc_created = (
                SupermarketCategory.objects.get_or_create(
                    name=sc_name, space=space, defaults=sm_category
                )
            )
        ...
        if onhand is not None:
            ...
            if onhand:
                ...
                validated_data["onhand_users"] = list(
                    set(onhand_users) - set(shared_users)
                )

        if properties_food_unit := validated_data.pop("properties_food_unit", None):
            properties_food_unit = Unit.objects.filter(
                name=properties_food_unit["name"]
            ).first()
    ...
    def update(self, instance, validated_data):
        ...
        if onhand is not None:
            ...
            if onhand:
                validated_data["onhand_users"] = (
                    list(self.instance.onhand_users.all()) + shared_users
                )
            else:
                validated_data["onhand_users"] = list(
...
class IngredientSimpleSerializer(WritableNestedModelSerializer):
    ...
    checked = serializers.BooleanField(
        read_only=True,
        default=False,
        help_text="Just laziness to have a checked field on the frontend API client",
    )
    ...
    def get_conversions(self, obj):
        if obj.unit and obj.food:
            ...
            for c in uch.get_conversions(obj):
                conversions.append(
                    {"food": c.food.name, "unit": c.unit.name, "amount": c.amount}
                )  # TODO do formatting in helper
            return conversions
        else:
            return []
...
class StepSerializer(WritableNestedModelSerializer, ExtendedRecipeMixin):
    ...
    instructions_markdown = serializers.SerializerMethodField(
        "get_instructions_markdown"
    )
    ...
    def get_step_recipe_data(self, obj):
        ...
        if obj.step_recipe and isinstance(self.parent.root, RecipeSerializer):
            return StepRecipeSerializer(
                obj.step_recipe, context={"request": self.context["request"]}
            ).data
...
class UnitConversionSerializer(WritableNestedModelSerializer, OpenDataModelMixin):
    ...
    def create(self, validated_data):
        validated_data["space"] = validated_data.pop(
            "space", self.context["request"].space
        )
        try:
            return UnitConversion.objects.get(
                food__name__iexact=validated_data.get("food", {}).get("name", None),
                base_unit__name__iexact=validated_data.get("base_unit", {}).get(
                    "name", None
                ),
                converted_unit__name__iexact=validated_data.get(
                    "converted_unit", {}
                ).get("name", None),
...
class RecipeBaseSerializer(WritableNestedModelSerializer):
    ...
    def is_recipe_new(self, obj):
        if getattr(obj, "new_recipe", None) or obj.created_at > (
...
class RecipeSerializer(RecipeBaseSerializer):
    ...
    def get_food_properties(self, obj):
        fph = FoodPropertyHelper(
            obj.space
        )  # initialize with object space since recipes might be viewed anonymously
...
class RecipeImageSerializer(WritableNestedModelSerializer):
    ...
    def create(self, validated_data):
        if "image" in validated_data and not is_file_type_allowed(
            validated_data["image"].name, image_only=True
        ):
            return None
        return super().create(validated_data)
...
class RecipeBookEntrySerializer(serializers.ModelSerializer):
    book_content = serializers.SerializerMethodField(
        method_name="get_book_content", read_only=True
    )
    recipe_content = serializers.SerializerMethodField(
        method_name="get_recipe_content", read_only=True
    )
    ...
    def get_book_content(self, obj):
        ...
        ).to_representation(obj.book)

    @extend_schema_field(RecipeOverviewSerializer)
    def get_recipe_content(self, obj):
        return RecipeOverviewSerializer(
            context={"request": self.context["request"]}
        ).to_representation(obj.recipe)

    def create(self, validated_data):
        book = validated_data["book"]
        recipe = validated_data["recipe"]
        if (
            not book.get_owner() == self.context["request"].user
            and not self.context["request"].user in book.get_shared()
        ):
...
class MealPlanSerializer(SpacedModelSerializer, WritableNestedModelSerializer):
    ...
    )  # TODO deprecate once old meal plan was removed
    ...
    def create(self, validated_data):
        ...
        if add_to_shopping and self.context["request"].data.get("recipe", None):
            SLR = RecipeShoppingEditor(
                user=validated_data["created_by"], space=validated_data["space"]
            )
...
class ShoppingListRecipeSerializer(serializers.ModelSerializer):
    recipe_data = RecipeOverviewSerializer(
        source="recipe", read_only=True, required=False
    )
    meal_plan_data = MealPlanSerializer(
        source="mealplan", read_only=True, required=False
    )
    servings = CustomDecimalField()
    created_by = UserSerializer(read_only=True)
    ...
    def update(self, instance, validated_data):
        if (
            ...
            and self.context.get("view", None).__class__.__name__
            != "ShoppingListViewSet"
        ):
            SLR = RecipeShoppingEditor(
                user=self.context["request"].user, space=self.context["request"].space
            )
...
class ShoppingListEntrySerializer(WritableNestedModelSerializer):
    ...
    list_recipe_data = ShoppingListRecipeSerializer(
        source="list_recipe", read_only=True
    )
    amount = CustomDecimalField()
    created_by = UserSerializer(read_only=True)
    completed_at = serializers.DateTimeField(allow_null=True, required=False)
    mealplan_id = serializers.IntegerField(
        required=False,
        write_only=True,
    ...
    def get_fields(self, *args, **kwargs):
        ...
        if self.context["request"] and bool(
            int(self.context["request"].query_params.get("autosync", False))
        ):
            for f in list(
                set(fields)
                - set(
    ...
    def create(self, validated_data):
        ...
        if "mealplan_id" in validated_data:
            if existing_slr := ShoppingListRecipe.objects.filter(
                mealplan_id=validated_data["mealplan_id"],
                space=self.context["request"].space,
            ).first():
                validated_data["list_recipe"] = existing_slr
            else:
                validated_data["list_recipe"] = ShoppingListRecipe.objects.create(
                    mealplan_id=validated_data["mealplan_id"],
                    space=self.context["request"].space,
                    created_by=self.context["request"].user,
                )
    ...
    def update(self, instance, validated_data):
        ...
        if user.userpreference.shopping_add_onhand:
            if checked := validated_data.get("checked", None):
                ...
                instance.food.onhand_users.add(
                    *user.userpreference.shopping_share.all(), user
                )
            elif not checked:
                instance.food.onhand_users.remove(
                    *user.userpreference.shopping_share.all(), user
                )
        return super().update(instance, validated_data)
...
class ViewLogSerializer(serializers.ModelSerializer):
    def create(self, validated_data):
        ...
        if not view_log:
            view_log = ViewLog.objects.create(
                recipe=validated_data["recipe"],
                created_by=self.context["request"].user,
                space=self.context["request"].space,
            )
            ...
            try:
                if (
                    InviteLink.objects.filter(
                        space=self.context["request"].space,
                        created_at__gte=datetime.now() - timedelta(hours=4),
                    ).count()
                    < 20
                ):
                    message = (
                        _("Hello")
                        + "!\n\n"
                        + _("You have been invited by ")
                        + escape(self.context["request"].user.get_user_display_name())
                    )
                    message += (
                        _(" to join their Tandoor Recipes space ")
                        + escape(self.context["request"].space.name)
                        + ".\n\n"
                    )
                    message += (
                        _("Click the following link to activate your account: ")
                    ...
                    message += (
                        _("The invitation is valid until ")
                        + str(obj.valid_until)
                        + "\n\n"
                    )
                    message += (
...
class FoodExportSerializer(FoodSerializer):
    supermarket_category = SupermarketCategoryExportSerializer(
        allow_null=True, required=False
    )
...
class RecipeFromSourceSerializer(serializers.Serializer):
    url = serializers.CharField(
        max_length=4096, required=False, allow_null=True, allow_blank=True
    )
...
class SourceImportRecipeSerializer(serializers.Serializer):
    ...
    properties = serializers.ListField(
        child=SourceImportPropertySerializer(), default=[]
    )
...
class RecipeFromSourceResponseSerializer(serializers.Serializer):
    ...
    images = serializers.ListField(
        child=serializers.CharField(), default=[], allow_null=False
    )
    error = serializers.BooleanField(default=False)
    msg = serializers.CharField(max_length=1024, default="")
    duplicates = serializers.ListField(
        child=SourceImportDuplicateSerializer(), default=[], allow_null=False
    )

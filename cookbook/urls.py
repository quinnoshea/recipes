...
from .views import api, import_export, telegram, views
...
router.register(
    r"property-type", api.PropertyTypeViewSet
)  # NOTE: if regenerating the legacy API these need renamed to food-property
...
router.register(
    r"supermarket-category-relation", api.SupermarketCategoryRelationViewSet
)
...
router.register(
    r"server-settings", api.ServerSettingsViewSet, basename="server-settings"
)
...
urlpatterns = [
    ...
    path(
        "abuse/<slug:token>", views.report_share_abuse, name="view_report_share_abuse"
    ),
    ...
    path(
        "view/recipe/<int:pk>", views.redirect_recipe_view, name="redirect_recipe_view"
    ),
    path(
        "view/recipe/<int:pk>/<slug:share>",
        views.redirect_recipe_share_view,
        name="redirect_recipe_share_view",
    ),
    path("api/import/", api.AppImportView.as_view(), name="view_import"),
    path("api/export/", api.AppExportView.as_view(), name="api_export"),
    path(
        "api/get_external_file_link/<int:pk>/",
        api.get_external_file_link,
        name="api_get_external_file_link",
    ),
    path(
        "api/get_recipe_file/<int:pk>/", api.get_recipe_file, name="api_get_recipe_file"
    ),
    path("api/sync_all/", api.sync_all, name="api_sync"),
    path(
        "api/recipe-from-source/",
        api.RecipeUrlImportView.as_view(),
        name="api_recipe_from_source",
    ),
    path("api/ai-import/", api.AiImportView.as_view(), name="api_ai_import"),
    path(
        "api/import-open-data/",
        api.ImportOpenData.as_view(),
        name="api_import_open_data",
    ),
    path(
        "api/ingredient-from-string/",
        api.ingredient_from_string,
    ...
    path(
        ...
        api.reset_food_inheritance,
        name="api_reset_food_inheritance",
    ),
    path(
        "api/switch-active-space/<int:space_id>/",
        api.switch_active_space,
        name="api_switch_active_space",
    ),
    path(
        "api/download-file/<int:file_id>/", api.download_file, name="api_download_file"
    ),
    ...
    path(
        "docs/swagger/",
        views.Swagger.as_view(url_name="openapi-schema"),
        name="docs_swagger",
    ),
    path(
        "openapi/",
        SpectacularAPIView.as_view(api_version=TANDOOR_VERSION),
        name="openapi-schema",
    ),

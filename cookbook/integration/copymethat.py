...
class CopyMeThat(Integration):
    ...
    def import_file_name_filter(self, zip_info_object):
        if DEBUG:
            print(
                ...
                zip_info_object.filename == "recipes.html",
            )
    ...
    def get_recipe_from_file(self, file):
        ...
        recipe = Recipe.objects.create(
            name=file.find("div", {"id": "name"}).text.strip()[:128],
            source_url=source,
        ...
        try:
            recipe.servings = parse_servings(
                file.find("a", {"id": "recipeYield"}).text.strip()
            )
            recipe.working_time = iso_duration_to_minutes(
                file.find("span", {"meta": "prepTime"}).text.strip()
            )
            recipe.waiting_time = iso_duration_to_minutes(
                file.find("span", {"meta": "cookTime"}).text.strip()
            )
        except AttributeError:
            pass
            ...
            if len(file.find("span", {"id": "starred"}).text.strip()) > 0:
                recipe.keywords.add(
                    Keyword.objects.get_or_create(
                        space=self.request.space, name=_("Favorite")
                    )[0]
                )
        except AttributeError:
            pass
        ...
        try:
            if len(file.find("span", {"id": "made_this"}).text.strip()) > 0:
                recipe.keywords.add(
                    Keyword.objects.get_or_create(
                        space=self.request.space, name=_("I made this")
                    )[0]
                )
        except AttributeError:
            pass
        ...
        if isinstance(ingredients, Tag):
            for ingredient in ingredients.children:
                if (
                    not isinstance(ingredient, Tag)
                    or not ingredient.text.strip()
                    or "recipeIngredient_spacer" in ingredient["class"]
                ):
                    continue
                if any(
                    x in ingredient["class"]
                    for x in ["recipeIngredient_subheader", "recipeIngredient_note"]
                ):
                    step.ingredients.add(
                        Ingredient.objects.create(
                            is_header=True,
                            note=ingredient.text.strip()[:256],
            ...
            for f in self.files:
                if ".zip" in f["name"]:
                    ...
                    self.import_recipe_image(
                        ...
                        BytesIO(
                            import_zip.read(
                                file.find("img", class_="recipeImage").get("src")
                            )
                        ),
                        filetype=".jpeg",
                    )

...
class Mealie(Integration):
    ...
    def import_file_name_filter(self, zip_info_object):
        return re.match(
            r"^recipes/([A-Za-z\d\s\-_()\[\]\u00C0-\u017F])+/([A-Za-z\d\s\-_()\[\]\u00C0-\u017F])+.json$",
            ...
            for k in recipe_json["tags"]:
                if "name" in k:
                    keyword, created = Keyword.objects.get_or_create(
                        name=k["name"].strip(), space=self.request.space
                    )
            ...
            if ".zip" in f["name"]:
                ...
                try:
                    self.import_recipe_image(
                        ...
                        BytesIO(
                            import_zip.read(
                                f'recipes/{recipe_json["slug"]}/images/min-original.webp'
                            )
                        ),
                        filetype=get_filetype(
                            f'recipes/{recipe_json["slug"]}/images/original'
                        ),
                    )
                except Exception:
                    pass

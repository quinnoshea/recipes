...
class NextcloudCookbook(Integration):
    ...
    def get_recipe_from_file(self, file):
        ...
        description = (
            ...
            else recipe_json["description"].strip()
        )
            ...
            try:
                recipe.keywords.add(
                    Keyword.objects.get_or_create(
                        space=self.request.space, name=recipe_json["recipeCategory"]
                    )[0]
                )
            except Exception:
                pass
            ...
            try:
                for x in recipe_json["keywords"].split(","):
                    if x.strip() != "":
                        recipe.keywords.add(
                            Keyword.objects.get_or_create(
                                space=self.request.space, name=x
                            )[0]
                        )
            except Exception:
                pass
            ...
            try:
                if "calories" in recipe_json["nutrition"]:
                    nutrition["calories"] = int(
                        re.search(r"\d+", recipe_json["nutrition"]["calories"]).group()
                    )
                if "proteinContent" in recipe_json["nutrition"]:
                    nutrition["proteins"] = int(
                        re.search(
                            r"\d+", recipe_json["nutrition"]["proteinContent"]
                        ).group()
                    )
                if "fatContent" in recipe_json["nutrition"]:
                    nutrition["fats"] = int(
                        re.search(
                            r"\d+", recipe_json["nutrition"]["fatContent"]
                        ).group()
                    )
                if "carbohydrateContent" in recipe_json["nutrition"]:
                    nutrition["carbohydrates"] = int(
                        re.search(
                            r"\d+", recipe_json["nutrition"]["carbohydrateContent"]
                        ).group()
                    )

                if nutrition != {}:
                    recipe.nutrition = NutritionInformation.objects.create(
                        **nutrition, space=self.request.space
                    )
                    recipe.save()
            except Exception:
                pass
    ...
    def get_files_from_recipes(self, recipes, el, cookie):
        ...
        for recipe in recipes:
            if recipe.internal and recipe.space == self.request.space:
                ...
                export_zip_obj.writestr(
                    f"{recipe.name}/{filename}", recipe_stream.getvalue()
                )
                recipe_stream.close()
                ...
                try:
                    ...
                    export_zip_obj.writestr(
                        f"{recipe.name}/full.jpg", self.getJPEG(imageByte)
                    )
                    export_zip_obj.writestr(
                        f"{recipe.name}/thumb.jpg", self.getThumb(171, imageByte)
                    )
                    export_zip_obj.writestr(
                        f"{recipe.name}/thumb16.jpg", self.getThumb(16, imageByte)
                    )
                except ValueError:
                    pass

...
class OpenEats(Integration):
    ...
    def get_recipe_from_file(self, file):
        ...
        recipe = Recipe.objects.create(
            ...
            waiting_time=file["cook_time"],
            working_time=file["prep_time"],
        )
        ...
        cuisine_keyword, created = Keyword.objects.get_or_create(
            name="Cuisine", space=self.request.space
        )
        if file["cuisine"] != "":
            keyword, created = Keyword.objects.get_or_create(
                name=file["cuisine"].strip(), space=self.request.space
            )
            if created:
                keyword.move(cuisine_keyword, pos="last-child")
            recipe.keywords.add(keyword)

        course_keyword, created = Keyword.objects.get_or_create(
            name="Course", space=self.request.space
        )
        if file["course"] != "":
            keyword, created = Keyword.objects.get_or_create(
                name=file["course"].strip(), space=self.request.space
            )
            if created:
        ...
        for tag in file["tags"]:
            keyword, created = Keyword.objects.get_or_create(
                name=tag.strip(), space=self.request.space
            )
            recipe.keywords.add(keyword)

        for comment in file["comments"]:
            Comment.objects.create(
                recipe=recipe, text=comment["text"], created_by=self.request.user
            )
        ...
        for o in recipe_json:
            if o["model"] == "rating.rating":
                recipe_dict[o["fields"]["recipe"]]["comments"].append(
                    {"text": o["fields"]["comment"], "rating": o["fields"]["rating"]}
                )
            if o["model"] == "ingredient.ingredient":
                ingredient = {
                    ...
                    "amount": round(
                        o["fields"]["numerator"] / o["fields"]["denominator"], 2
                    ),
                }
                recipe_dict[ingredient_group_dict[o["fields"]["ingredient_group"]]][
                    "ingredients"
                ].append(ingredient)

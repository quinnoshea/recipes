...
class CookBookApp(Integration):
    ...
    def get_recipe_from_file(self, file):
        ...
        scrape = scrape_html(
            html=recipe_html, org_url="https://cookbookapp.import", supported_only=False
        )
        ...
        step = Step.objects.create(
            instruction=recipe_json["steps"][0]["instruction"],
            space=self.request.space,
        ...
        for ingredient in recipe_json["steps"][0]["ingredients"]:
            ...
            step.ingredients.add(
                Ingredient.objects.create(
                    ...
                    original_text=ingredient.get("original_text", None),
                    space=self.request.space,
                )
            )
        ...
        try:
            for url in images:
                ...
                if validate_import_url(url) and not url.startswith(
                    "https://media.cookbookmanager.com/brand/"
                ):

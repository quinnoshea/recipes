...
class RecetteTek(Integration):
    ...
    def import_file_name_filter(self, zip_info_object):
        ...
        return re.match(r"^recipes_0.json$", zip_info_object.filename) or re.match(
            r"^recipes.json$", zip_info_object.filename
        )
                    ...
                    if validate_import_url(url):
                        ...
                        if Image.open(BytesIO(response.content)).verify():
                            self.import_recipe_image(
                                ...
                                filetype=get_filetype(file["originalPicture"]),
                            )
                        else:

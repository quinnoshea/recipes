        ...
        if images:
            try:
                self.import_recipe_image(
                    recipe,
                    BytesIO(recipe_zip.read(images[0])),
                    filetype=get_filetype(images[0]),
                )
    ...
    def get_files_from_recipes(self, recipes, el, cookie):
        ...
        for r in recipes:
            if r.internal and r.space == self.request.space:
                ...
                try:
                    recipe_zip_obj.writestr(
                        f"image{get_filetype(r.image.file.name)}", r.image.file.read()
                    )
                ...
                export_zip_obj.writestr(
                    str(r.pk) + ".zip", recipe_zip_stream.getvalue()
                )

        ...
        with gzip.open(file, "r") as recipe_zip:
            ...
            try:
                ...
                if len(recipe_json["cook_time"].strip()) > 0:
                    recipe.waiting_time = re.findall(r"\d+", recipe_json["cook_time"])[
                        0
                    ]

                if len(recipe_json["prep_time"].strip()) > 0:
                    recipe.working_time = re.findall(r"\d+", recipe_json["prep_time"])[
                        0
                    ]
            except Exception:
                pass
            ...
            if (
                ...
                and len(recipe_json["nutritional_info"].strip()) > 0
            ):
                instructions += (
                    "\n\n### "
            ...
            try:
                if (
                    len(recipe_json["source"].strip()) > 0
                    or len(recipe_json["source_url"].strip()) > 0
                ):
                    instructions += (
                        "\n\n### "
                        + _("Source")
            ...
            if (
                "description" in recipe_json
                and len(recipe_json["description"].strip()) > 500
            ):
                step.instruction = (

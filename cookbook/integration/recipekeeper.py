...
class RecipeKeeper(Integration):
    ...
    def get_recipe_from_file(self, file):
        ...
        recipe = Recipe.objects.create(
            name=file.find("h2", {"itemprop": "name"}).text.strip(),
        ...
        if file.find("span", {"itemprop": "recipeSource"}).text != "":
            step.instruction += (
                ...
                + _("Imported from")
                + ": "
                + file.find("span", {"itemprop": "recipeSource"}).text
            )
            step.save()
            ...
            for f in self.files:
                if ".zip" in f["name"]:
                    ...
                    self.import_recipe_image(
                        ...
                        BytesIO(
                            import_zip.read(
                                file.find("img", class_="recipe-photo").get("src")
                            )
                        ),
                        filetype=".jpeg",
                    )

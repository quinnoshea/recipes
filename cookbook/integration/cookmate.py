...
class Cookmate(Integration):
    ...
    def get_recipe_from_file(self, file):
        ...
        if (
            recipe_xml.find("preptime") is not None
            and recipe_xml.find("preptime").text is not None
        ):
            recipe.working_time = parse_time(recipe_xml.find("preptime").text.strip())

        if (
            recipe_xml.find("cooktime") is not None
            and recipe_xml.find("cooktime").text is not None
        ):
            recipe.waiting_time = parse_time(recipe_xml.find("cooktime").text.strip())

        if (
            recipe_xml.find("quantity") is not None
            and recipe_xml.find("quantity").text is not None
        ):
        ...
        if (
            recipe_xml.find("url") is not None
            and recipe_xml.find("url").text is not None
        ):
            recipe.source_url = recipe_xml.find("url").text.strip()

        if (
            recipe_xml.find("description") is not None

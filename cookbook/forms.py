...
class ImportExportBase(forms.Form):
    ...
    type = forms.ChoiceField(
        choices=(
            ...
            (PEPPERPLATE, "Pepperplate"),
            (RECETTETEK, "RecetteTek"),
            (RECIPESAGE, "Recipe Sage"),
            (DOMESTICA, "Domestica"),
            (MEALMASTER, "MealMaster"),
            (REZKONV, "RezKonv"),
            (OPENEATS, "Openeats"),
            (RECIPEKEEPER, "Recipe Keeper"),
            (PLANTOEAT, "Plantoeat"),
            (COOKBOOKAPP, "CookBookApp"),
            (COPYMETHAT, "CopyMeThat"),
            (PDF, "PDF"),
...
class ExportForm(ImportExportBase):
    recipes = forms.ModelMultipleChoiceField(
        widget=MultiSelectWidget, queryset=Recipe.objects.none(), required=False
    )
...
class InviteLinkForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (
            space.max_users != 0
            and (
                UserPreference.objects.filter(space=space).count()
                + InviteLink.objects.filter(
                    valid_until__gte=datetime.today(), used_by=None, space=space
                ).count()
            )
            >= space.max_users
        ):
    ...
    class Meta:
        ...
        help_texts = {
            "email": _(
                "An email address is not required but if present the invite link will be sent to the user."
            ),
        }
        field_classes = {

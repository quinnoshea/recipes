...
DRF_THROTTLE_RECIPE_URL_IMPORT = os.getenv(
    "DRF_THROTTLE_RECIPE_URL_IMPORT",
    os.getenv("RATELIMIT_URL_IMPORT_REQUESTS", "60/hour"),
)
...
def setup_database(
    ...
    if DATABASE_URL:
        ...
        if schema.startswith("postgres"):
            ...
            if (
                db_path := os.path.dirname(settings["database"])
...
if REDIS_HOST:
    ...
    if REDIS_USERNAME and not REDIS_PASSWORD:
        CACHES["default"][
            "LOCATION"
        ] = f"redis://{REDIS_USERNAME}@{REDIS_HOST}:{REDIS_PORT}"
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    try:
        s.connect(
            (
                DJANGO_VITE["default"]["dev_server_host"],
                DJANGO_VITE["default"]["dev_server_port"],
            )
        )
        if DEBUG:
...
if os.getenv("TIMEZONE") is not None:
    print(
        'DEPRECATION WARNING: Environment var "TIMEZONE" is deprecated. Please use "TZ" instead.'
    )

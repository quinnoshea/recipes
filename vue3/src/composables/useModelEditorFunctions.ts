...
export function useModelEditorFunctions<T>(modelName: EditorSupportedModels, emit: any) {
  ...
  function setupState(
    ...
    if (item === null && (itemId === undefined || itemId == '')) {
      ...
      if (modelClass.value.model.disableCreate) {
        throw Error(
          'Trying to use a ModelEditor without an item and a model that does not allow object creation!'
        )
      }
  ...
  function saveObject() {
    ...
    if (isUpdate()) {
      return modelClass.value
        .update(editingObj.value.id, editingObj.value)
        .then((r: T) => {
          emit('save', r)
          editingObj.value = r

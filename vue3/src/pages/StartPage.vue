<template>
  <v-container>
    <horizontal-meal-plan-window
      v-if="useUserPreferenceStore().deviceSettings.start_showMealPlan"
    ></horizontal-meal-plan-window>

    <v-card v-if="totalRecipes == 0" class="mt-5 mb-5">
      <v-card-title
        ><i class="fa-solid fa-eye-slash"></i> {{ $t('search_no_recipes') }}</v-card-title
      >
      <v-card-text>
        <v-btn-group divided>
          <v-btn
    ...
    <v-row>
      <v-col class="text-center">
        <v-btn
          ...
          prepend-icon="$search"
          variant="tonal"
          :to="{ name: 'SearchPage', params: { query: '' } }"
          >{{ $t('View_Recipes') }}</v-btn
        >
      </v-col>
    </v-row>
  </v-container>

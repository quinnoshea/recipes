<template>
  <v-container>
        ...
        >
          <template v-slot:append>
            <v-badge
              ...
              :offset-y="5"
              color="secondary"
              v-model="hasFiltersApplied"
            >
              <v-btn @click="panel = 'search'" v-if="panel == ''" color="primary" icon>
                <i class="fa-solid fa-caret-down"></i
              ></v-btn>
              <v-btn @click="panel = ''" v-if="panel == 'search'" color="primary" icon
        ...
        <v-expansion-panels v-model="panel">
          <v-expansion-panel value="search">
                ...
                <model-select model="CustomFilter" v-model="selectedCustomFilter" density="compact">
                  <template #append>
                    <v-btn
                      ...
                      @click="loadSelectedCustomFilter()"
                    ></v-btn>
                    <v-btn
                      icon="$save"
                      class="ms-1"
            ...
            <v-card-actions v-if="panel == 'search'">
              <v-btn @click="reset()" prepend-icon="fa-solid fa-circle-xmark">{{
                $t('Reset')
              }}</v-btn>
              <v-btn @click="searchRecipes({ page: 1 })" prepend-icon="$search">{{
                $t('Search')
              }}</v-btn>
            </v-card-actions>
          </v-expansion-panel>
    ...
    <v-row
      v-if="
        recipes.length > 0 && useUserPreferenceStore().deviceSettings.search_viewMode == 'table'
      "
    >
      <v-col>
        <v-card>
    ...
    <template
      v-if="recipes.length > 0 && useUserPreferenceStore().deviceSettings.search_viewMode == 'grid'"
    >
      <v-row>
    ...
    <v-dialog v-model="dialog">
      <v-card>
        ...
        <v-card-actions>
          <v-btn prepend-icon="$create" color="create" @click="createCustomFilter()">{{
            $t('Create')
          }}</v-btn>
        </v-card-actions>
      </v-card>
    ...
    <batch-delete-dialog
      ...
      activator="model"
      @change="searchRecipes({ page: 1 })"
    ></batch-delete-dialog>
    <batch-edit-recipe-dialog
      :items="selectedItems"
      v-model="batchEditDialog"
      activator="model"
      @change="searchRecipes({ page: page })"
    ></batch-edit-recipe-dialog>
  </v-container>
</template>
...
const pageSize = useRouteQuery(
  'pageSize',
  useUserPreferenceStore().deviceSettings.general_tableItemsPerPage,
  { transform: Number }
)
...
onMounted(() => {
  ...
  useUserPreferenceStore().deviceSettings.search_visibleFilters.forEach((f) => {
    if (f in filters.value) {
      ...
      useUserPreferenceStore().deviceSettings.search_visibleFilters.splice(
        useUserPreferenceStore().deviceSettings.search_visibleFilters.indexOf(f),
        1
      )
    }
  })
  ...
  if (selectedCustomFilter.value != null) {
    ...
    api
      .apiCustomFilterUpdate({
        id: selectedCustomFilter.value.id!,
        customFilter: selectedCustomFilter.value,
      })
      .then((r) => {
...
function createCustomFilter() {
  ...
  loading.value = true
  api
    .apiCustomFilterCreate({
      customFilter: {
        name: newFilterName.value,
        search: JSON.stringify(filtersToCustomFilterFormat()),
      } as CustomFilter,
    })
    .then((r) => {
      selectedCustomFilter.value = r

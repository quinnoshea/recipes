<template>
  <v-container>
    <v-card :loading="recipeLoading || propertyTypesLoading">
      ...
      <v-card-text>
        <model-select
          ...
          model="Recipe"
          v-model="recipe"
          @update:model-value="loadRecipe(recipe.id!)"
        >
          <template #append>
            <v-btn
              icon="fa-solid fa-arrow-up-right-from-square"
              :to="{ name: 'RecipeViewPage', params: { id: recipe.id } }"
            ...
            <tr>
              ...
              <th>
                <v-btn
                  variant="outlined"
                  block
                  href="https://fdc.nal.usda.gov/food-search"
                  target="_blank"
                  prepend-icon="$search"
                  stacked
                  >{{ $t('FDC_ID') }}</v-btn
              ...
              <th>
                <v-btn variant="outlined" @click="dialog = true" block stacked>{{
                  $t('Amount')
                }}</v-btn>
              </th>
              <th>
                ...
                <v-btn stacked block variant="outlined" class="mt-2 mb-2">
                  ...
                  <span>
                    <v-chip color="info" size="x-small"
                      ><v-icon icon="fa-solid fa-arrow-down-1-9"></v-icon>{{ pt.order }}</v-chip
                    >
                    <v-chip color="success" size="x-small" v-if="pt.fdcId"
                      ><v-icon icon="fa-solid fa-check"></v-icon>FDC</v-chip
                    >
                    <v-chip color="error" size="x-small" v-if="!pt.fdcId"
                      ><v-icon icon="fa-solid fa-times"></v-icon>FDC</v-chip
                    >
                  </span>
              ...
              <th>
                <v-btn color="create" class="mt-1 float-right">
                  ...
                  <model-edit-dialog
                    model="PropertyType"
                    @create="(pt: PropertyType) => propertyTypes.push(pt)"
                  ></model-edit-dialog>
                </v-btn>
              </th>
            </tr>
          ...
          <tbody>
            <tr v-for="[i, ingredient] in ingredients.entries()">
              ...
              <td>
                <v-text-field
                  type="number"
                  v-model="ingredient.food.fdcId"
                  density="compact"
                  hide-details
                  @change="updateFood(ingredient)"
                ...
                >
                  <template #append-inner>
                    <v-btn
                      ...
                      density="compact"
                      variant="plain"
                      v-if="ingredient.food.fdcId == undefined"
                      @click="
                        fdcSelectedIngredient = ingredient
                        fdcDialog = true
                      "
                    ></v-btn>
                    <v-btn
                      @click="updateFoodFdcData(ingredient)"
                      icon="fa-solid fa-arrows-rotate"
                      size="small"
                      density="compact"
                      variant="plain"
                      v-if="ingredient.food.fdcId"
                    ></v-btn>
                    <v-btn
                      @click="openFdcPage(ingredient.food.fdcId)"
                      :href="`https://fdc.nal.usda.gov/food-details/${ingredient.food.fdcId}/nutrients`"
                      target="_blank"
                      icon="fa-solid fa-arrow-up-right-from-square"
                      size="small"
                      variant="plain"
              ...
              <td>
                <v-number-input
                  v-model="ingredient.food.propertiesFoodAmount"
                  density="compact"
                  hide-details
                  @change="updateFood(ingredient)"
                  :loading="ingredient.loading"
              ...
              <td>
                <model-select
                  model="Unit"
                  density="compact"
                  v-model="ingredient.food.propertiesFoodUnit"
                  hide-details
                  @update:model-value="updateFood(ingredient)"
                  :loading="ingredient.loading"
                ></model-select>
              </td>
              <td
                v-for="p in ingredient.food.properties"
                v-bind:key="`${ingredient.food.id}_${p.propertyType.id}`"
              >
                <v-number-input
                  v-model="p.propertyAmount"
                  density="compact"
                  hide-details
                  v-if="p.propertyAmount != null"
                  @change="updateFood(ingredient)"
                  :precision="2"
                  :loading="ingredient.loading"
                  @click:clear="deleteFoodProperty(p, ingredient)"
    ...
    <fdc-search-dialog
      ...
      @selected="
        (fdcId: number) => {
          fdcSelectedIngredient.food.fdcId = fdcId
          updateFoodFdcData(fdcSelectedIngredient)
        }
      "
    ></fdc-search-dialog>
  </v-container>
      ...
      <v-card-text>
        ...
        <model-select model="Unit" :label="$t('Properties_Food_Unit')" v-model="dialogUnit">
          <template v-slot:append>
            <v-btn
              @click="changeAllUnits(dialogUnit)"
              icon="$save"
              color="save"
              :disabled="dialogUnit == undefined"
            ></v-btn>
          </template>
        </model-select>
        ...
        <v-text-field type="number" :label="$t('Properties_Food_Amount')" v-model="dialogAmount">
          <template v-slot:append>
            <v-btn
              @click="changeAllPropertyFoodAmounts(dialogAmount)"
              icon="$save"
              color="save"
            ></v-btn>
          </template>
...
const calculatorToNumerator = computed(() => {
  return (
    (calculatorFromNumerator.value / calculatorFromDenominator.value) *
    calculatorToDenominator.value
  ).toFixed(2)
})
...
function updateFood(ingredient: IngredientLoading) {
  ...
  ingredient.loading = true
  api
    .apiFoodPartialUpdate({ id: ingredient.food.id!, patchedFood: ingredient.food })
    .then((r) => {})
    .catch((err) => {
...
function updateFoodFdcData(ingredient: IngredientLoading) {
  ...
  if (ingredient.food.fdcId) {
    ingredient.loading = true
    api
      .apiFoodFdcCreate({ id: ingredient.food.id!, food: ingredient.food })
      .then((r) => {
        ingredient.food = r

            ...
            >
              <template #append>
                    ...
                    <v-list density="compact">
                      ...
                      <v-list-item link prepend-icon="$automation" :disabled="!selectedFood">
                        ...
                        <model-edit-dialog
                          model="Automation"
                          activator="parent"
                          :item-defaults="{ param1: selectedFood.name, type: 'FOOD_ALIAS' }"
                          v-if="selectedFood"
                        ></model-edit-dialog>
                      </v-list-item>
                ...
                <v-btn
                  ...
                  :to="{ name: 'ModelListPage', params: { model: 'food' } }"
                  variant="plain"
                ></v-btn>
              </template>
            </model-select>
            ...
            >
              <template #append>
                    ...
                    <v-list density="compact">
                      ...
                      <v-list-item link prepend-icon="$automation" :disabled="!selectedUnit">
                        ...
                        <model-edit-dialog
                          model="Automation"
                          activator="parent"
                          :item-defaults="{ param1: selectedUnit.name, type: 'UNIT_ALIAS' }"
                          v-if="selectedUnit"
                        ></model-edit-dialog>
                      </v-list-item>
                ...
                <v-btn
                  ...
                  :to="{ name: 'ModelListPage', params: { model: 'unit' } }"
                  variant="plain"
                ></v-btn>
              </template>
            </model-select>
        ...
        <template v-slot:item.action="{ item }">
          <v-btn-group density="comfortable">
            ...
            <v-btn size="small" color="delete" :loading="item.loading">
              ...
              <delete-confirm-dialog
                :model-name="$t('Ingredient')"
                @delete="deleteIngredient(item)"
              ></delete-confirm-dialog>
            </v-btn>
          </v-btn-group>
        </template>
...
function updateIngredient(ingredient: EditorIngredient) {
  ...
  api
    .apiIngredientUpdate({ id: ingredient.id!, ingredient: ingredient })
    .then((r) => {})
    .catch((err) => {
...
function deleteIngredient(ingredient: EditorIngredient) {
  ...
  ingredient.loading = true
  api
    .apiIngredientDestroy({ id: ingredient.id! })
    .then((r) => {
...
function getAndLoadParameters() {
  ...
  if (params.food_id && !Number.isNaN(params.food_id)) {
    promises.push(
      api.apiFoodRetrieve({ id: Number(params.food_id) }).then((r) => {
  ...
  if (params.unit_id && !Number.isNaN(params.unit_id)) {
    promises.push(
      api.apiUnitRetrieve({ id: Number(params.unit_id) }).then((r) => {
...
function loadItems({ page, itemsPerPage, search, sortBy, groupBy }) {
  ...
  api
    .apiIngredientList(requestParameters)
    .then((r) => {
...
function deleteFood() {
  ...
  if (selectedFood.value) {
    filtersLoading.value = true
    api
      .apiFoodDestroy({ id: selectedFood.value.id! })
      .then((r) => {
...
function deleteUnit() {
  ...
  if (selectedUnit.value) {
    filtersLoading.value = true
    api
      .apiUnitDestroy({ id: selectedUnit.value.id! })
      .then((r) => {

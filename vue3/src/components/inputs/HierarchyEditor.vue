<template>
  <v-row justify="space-between" dense>
    ...
    <v-col cols="6">
      <v-card
        v-if="activeObjs.length == 1"
        :title="activeObjs[0].name"
      ...
      >
        <v-card-text>
          ...
          <model-select :model="genericModel.model.name" v-model="addChildObj" allow-create>
            <template #append>
              <v-btn
                color="save"
                icon="$save"
                :disabled="addChildObj == undefined"
                @click="
                  moveObject(addChildObj, activeObjs[0].id!)
                  addChildObj = undefined
                "
              ></v-btn>
            </template>
          </model-select>
          ...
          <model-select :model="genericModel.model.name" v-model="setParentObj" allow-create>
            <template #append>
              <v-btn
                color="save"
                icon="$save"
                :disabled="setParentObj == undefined"
                @click="
                  moveObject(activeObjs[0], setParentObj.id!)
                  setParentObj = undefined
                "
              ></v-btn>
            </template>
          </model-select>
          ...
          <v-btn
            @click="moveObject(activeObjs[0], 0)"
            class="mt-2"
            color="warning"
          ...
          <v-btn
            ...
            :to="{
              name: 'ModelEditPage',
              params: { model: genericModel.model.name, id: activeObjs[0].id },
            }"

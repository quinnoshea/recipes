<template>
  ...
  >
    ...
    <Multiselect
      ...
      :class="{
        'model-select--density-compact': props.density == 'compact',
        'model-select--density-comfortable': props.density == 'comfortable',
        'model-select--density-default': props.density == '',
      }"
    ...
    >
      ...
      <template v-if="hasMoreItems && !loading" #afterlist>
        <span class="text-disabled font-italic text-caption ms-3">{{
...
function search(query: string) {
  ...
  return modelClass.value
    .list({ query: query, page: 1, pageSize: props.limit })
    .then((r: any) => {
...
async function createObject(object: any, select$: Multiselect) {
  return await modelClass.value
    .create({ name: object[itemLabel.value] })
    .then((createdObj: any) => {

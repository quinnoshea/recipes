<template>
  <v-btn-group density="compact">
    <v-btn color="create" @click="properties.push({} as Property)" prepend-icon="$create">{{
      $t('Add')
    }}</v-btn>
    <v-btn color="secondary" @click="addAllProperties" prepend-icon="fa-solid fa-list">{{
      $t('AddAll')
    }}</v-btn>
  </v-btn-group>
    ...
    <v-col cols="0" md="6">
      ...
      >
        <template #append-inner v-if="p.propertyType">
          <v-chip class="me-4">{{ p.propertyType.unit }} / {{ props.amountFor }} </v-chip>
        </template>
      </v-number-input>
    </v-col>
    <v-col cols="0" md="6">

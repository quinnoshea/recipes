<template>
  <slot name="activator">
    ...
    <v-btn @click="dialog = true" variant="plain" class="d-print-none" v-else>
      ...
      <v-chip size="x-small" variant="tonal" class="d-none d-md-flex ml-1" label>{{
        $t('Ctrl+K')
      }}</v-chip>
    </v-btn>
  </slot>
  ...
  >
    <v-card>
      ...
      <v-card-text class="pt-0 pt-md-2">
        ...
        >
          <v-card-title @click="goToSelectedRecipe(index)">
            ...
            <v-avatar v-else-if="item.recipeId !== undefined" color="tandoor">{{
              item.name.charAt(0)
            }}</v-avatar>
      ...
      <v-card-text class="d-none d-sm-block pt-2">
        ...
        <v-chip size="x-small" class="mr-1" label
...
const searchResults = computed(() => {
  ...
  if (searchQuery.value != '' && searchQuery.value != null) {
    flatRecipes.value
      .filter((fr) => fr.name.toLowerCase().includes(searchQuery.value.toLowerCase()))
      .slice(0, 10)
      .forEach((r) => {
        searchResults.push({
          name: r.name,
...
const debouncedAsyncSearch = useDebounceFn(() => {
  if (searchQuery.value != null && searchQuery.value != '') {
    ...
    api
      .apiRecipeList({ query: searchQuery.value })
      .then((r) => {

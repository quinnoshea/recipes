  ...
  <v-tabs v-model="currentTab">
    <v-tab value="shopping"
      ><i
        class="fas fa-fw"
        :class="{
          'fa-circle-notch fa-spin': useShoppingStore().currentlyUpdating,
          'fa-shopping-cart ': !useShoppingStore().currentlyUpdating,
        }"
      ></i>
      <span class="d-none d-md-block ms-1"
        >{{ $t('Shopping_list') }} ({{ useShoppingStore().stats.countUnchecked }})</span
      ></v-tab
    >
    <v-tab value="recipes"
      ><i class="fas fa-book fa-fw"></i>
      <span class="d-none d-md-block ms-1"
        >{{ $t('Recipes') }} ({{ useShoppingStore().getAssociatedRecipes().length }})</span
      ></v-tab
    >
    <v-tab
      value="selected_supermarket"
      v-if="useUserPreferenceStore().deviceSettings.shopping_selected_supermarket != null"
    >
    ...
    <v-menu :close-on-content-click="false">
      ...
      <v-list density="compact">
        <v-list-item
          @click="useShoppingStore().undoChange()"
        ...
        <v-list-item>
          <v-select
            hide-details
            :items="groupingOptionsItems"
            v-model="useUserPreferenceStore().deviceSettings.shopping_selected_grouping"
            :label="$t('GroupBy')"
          >
          </v-select>
        </v-list-item>
        <v-list-item
          v-if="
            useUserPreferenceStore().deviceSettings.shopping_selected_grouping ==
            ShoppingGroupingOptions.CATEGORY
          "
        >
          <v-switch
            color="primary"
            ...
            v-model="
              useUserPreferenceStore().deviceSettings.shopping_show_selected_supermarket_only
            "
          ></v-switch>
        </v-list-item>
        <v-list-item>
          <model-select
            model="Supermarket"
            append-to-body
            v-model="useUserPreferenceStore().deviceSettings.shopping_selected_supermarket"
          ></model-select>
        </v-list-item>
        ...
        <v-list-item>
          <v-switch
            ...
            hide-details
            :label="$t('ShowDelayed')"
            v-model="useUserPreferenceStore().deviceSettings.shopping_show_delayed_entries"
          ></v-switch>
        </v-list-item>
        <v-list-item>
          <v-switch
            color="primary"
            hide-details
            :label="$t('ShowRecentlyCompleted')"
            v-model="useUserPreferenceStore().deviceSettings.shopping_show_checked_entries"
          ></v-switch>
        </v-list-item>
        ...
        <v-list-item>
          <v-switch
            ...
            hide-details
            :label="$t('Recipe')"
            v-model="useUserPreferenceStore().deviceSettings.shopping_item_info_recipe"
          ></v-switch>
        </v-list-item>
        <v-list-item>
          <v-switch
            ...
            hide-details
            :label="$t('Meal_Plan')"
            v-model="useUserPreferenceStore().deviceSettings.shopping_item_info_mealplan"
          ></v-switch>
        </v-list-item>
        <v-list-item>
          <v-switch
            ...
            hide-details
            :label="$t('CreatedBy')"
            v-model="useUserPreferenceStore().deviceSettings.shopping_item_info_created_by"
          ></v-switch>
        </v-list-item>
        <v-list-item>
          <v-switch
            ...
            hide-details
            label="New Input"
            v-model="useUserPreferenceStore().deviceSettings.shopping_input_autocomplete"
          ></v-switch>
        </v-list-item>
        <v-list-item v-if="useUserPreferenceStore().serverSettings.debug">
          <v-switch
            ...
            v-model="useUserPreferenceStore().deviceSettings.shopping_show_debug"
          ></v-switch>
        </v-list-item>
      </v-list>
    </v-menu>
        ...
        <v-row>
          <v-col>
            ...
            <v-list class="mt-3" density="compact" v-else>
              <template
                v-for="category in useShoppingStore().getEntriesByGroup"
                :key="category.name"
              >
                <template v-if="isShoppingCategoryVisible(category)">
                  <v-list-subheader v-if="category.name === useShoppingStore().UNDEFINED_CATEGORY"
        ...
        <v-row v-if="useUserPreferenceStore().deviceSettings.shopping_show_debug">
          <v-col cols="12" md="4">
            <v-card>
              ...
              <v-card-text>
                <v-list>
                  <v-list-item
                    >currentlyUpdating: {{ useShoppingStore().currentlyUpdating }}</v-list-item
                  >
                  <v-list-item>hasFocus: {{ useShoppingStore().autoSyncHasFocus }}</v-list-item>
                  <v-list-item
                    >autoSyncTimeoutId: {{ useShoppingStore().autoSyncTimeoutId }}</v-list-item
                  >
                  <v-list-item
                    >autoSyncLastTimestamp:
                    {{ useShoppingStore().autoSyncLastTimestamp }}</v-list-item
                  >
                </v-list>
              </v-card-text>
            </v-card>
          ...
          <v-col cols="12" md="4">
            <v-card>
              ...
              <v-card-text>
                ...
                <v-list>
                  <v-list-item v-for="i in useShoppingStore().itemCheckSyncQueue" :key="i">{{
                    i
                  }}</v-list-item>
                </v-list>
              </v-card-text>
          ...
          <v-col cols="12" md="4">
            <v-card>
              ...
              <v-card-text>
                <v-list>
                  <v-list-item v-for="i in useShoppingStore().undoStack" :key="i"
                    >{{ i.type }} {{ i.entries.flatMap((e: ShoppingListEntry) => e.food.name) }}
                  </v-list-item>
                </v-list>
              </v-card-text>
            </v-card>
                ...
                <v-list>
                  <v-list-item v-for="r in useShoppingStore().getAssociatedRecipes()">
                    ...
                    <div class="ms-2">
                      ...
                      <p v-if="r.mealplan">
                        {{ r.mealPlanData.mealType.name }} -
                        {{
                          DateTime.fromJSDate(r.mealPlanData.fromDate).toLocaleString(
                            DateTime.DATE_FULL
                          )
                        }}
                        #{{ r.id }}
...
onMounted(() => {
  ...
  if (useUserPreferenceStore().deviceSettings.shopping_selected_supermarket != null) {
    new ApiApi()
      .apiSupermarketRetrieve({
        id: useUserPreferenceStore().deviceSettings.shopping_selected_supermarket!.id!,
      })
      .then((r) => {

    ...
    <v-card-text>
      <v-list>
        <v-list-item
          ...
          v-for="c in cookLogs"
          :key="c.id"
          :link="c.createdBy.id == useUserPreferenceStore().userSettings?.user.id"
        >
          <template #prepend>
          ...
          <v-list-item-subtitle
            ...
            v-if="c.servings != null && c.servings > 0"
          >
            {{ c.servings }}
          ...
          <template #append>
            <v-list-item-action class="flex-column align-end">
              <v-rating
                ...
                half-increments
                readonly
                v-if="c.rating != undefined"
              ></v-rating>
              <v-spacer></v-spacer>
              <v-tooltip
                location="top"
                :text="DateTime.fromJSDate(c.createdAt).toLocaleString(DateTime.DATETIME_MED)"
                v-if="c.createdAt != undefined"
              >
                <template v-slot:activator="{ props }">
                  <span v-bind="props">{{
                    DateTime.fromJSDate(c.createdAt).toRelative({ style: 'narrow' })
                  }}</span>
                </template>
              </v-tooltip>
            </v-list-item-action>
          ...
          <model-edit-dialog
            model="CookLog"
            :item="c"
            v-if="c.createdBy.id == useUserPreferenceStore().userSettings?.user.id"
            @save="recLoadCookLog(props.recipe.id)"
            @delete="recLoadCookLog(props.recipe.id)"
          ></model-edit-dialog>
        </v-list-item>
      </v-list>
    </v-card-text>
...
function saveCookLog() {
  ...
  api
    .apiCookLogCreate({ cookLog: newCookLog.value })
    .then((r) => {

          ...
          <v-row>
            <v-col v-for="mealPlanGridItem in w">
              <v-list density="compact" class="pt-0 pb-0">
                <v-list-item class="text-center">
                  <div class="d-flex">
                    <div class="flex-col align-self-start">
                      <v-btn
                        @click="currentWindowIndex--"
                        v-if="currentWindowIndex != 0"
                    ...
                    <div class="flex-col align-self-end">
                      <v-btn
                        @click="currentWindowIndex++"
                        v-if="currentWindowIndex + 1 < mealPlanWindows.length"
                ...
                <v-list-item
                  v-for="p in mealPlanGridItem.plan_entries"
                  :key="p.id"
                  @click="clickMealPlan(p)"
                  link
                >
                  <template #prepend>
    ...
    grid.push({
      ...
      plan_entries: useMealPlanStore().planList.filter(
        (m: MealPlan) =>
          ...
          DateTime.fromJSDate(m.toDate != undefined ? m.toDate : m.fromDate).startOf('day') >=
            grid_day_date.startOf('day')
      ),
    } as MealPlanGridItem)
  }

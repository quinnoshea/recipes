      ...
      <v-card class="h-100" :loading="useMealPlanStore().loading">
        ...
        <calendar-view
          ...
          :display-period-count="
            useUserPreferenceStore().deviceSettings.mealplan_displayPeriodCount
          "
          :starting-day-of-week="useUserPreferenceStore().deviceSettings.mealplan_startingDayOfWeek"
          :display-week-numbers="
            useUserPreferenceStore().deviceSettings.mealplan_displayWeekNumbers
          "
 ...
 * @param event
 */
function dropCalendarItemOnDate(
  undefinedItem: IMealPlanNormalizedCalendarItem,
  targetDate: Date,
  event: DragEvent
) {
  ...
  if (currentlyDraggedMealplan.value.originalItem.mealPlan.id != undefined) {
    let mealPlan = useMealPlanStore().plans.get(
      currentlyDraggedMealplan.value.originalItem.mealPlan.id
    )
    if (mealPlan != undefined) {
      let fromToDiff = { days: 0 }
      if (mealPlan.toDate && mealPlan.toDate > mealPlan.fromDate) {
        fromToDiff = DateTime.fromJSDate(mealPlan.toDate).diff(
          DateTime.fromJSDate(mealPlan.fromDate),
          'days'
        )
      }

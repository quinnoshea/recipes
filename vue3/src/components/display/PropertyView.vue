  ...
  <v-card class="mt-2" v-if="hasFoodProperties || hasRecipeProperties">
    <v-card-title>
      ...
      <v-btn-toggle
        ...
        v-if="hasRecipeProperties && hasRecipeProperties"
        v-model="sourceSelectedToShow"
      >
  ...
  <v-dialog max-width="900px" v-model="dialog">
    <v-card v-if="dialogProperty" :loading="loading">
      <v-closable-card-title
        :title="`${dialogProperty.propertyAmountTotal} ${dialogProperty.unit} ${dialogProperty.name}`"
        :sub-title="$t('total')"
        icon="$properties"
        v-model="dialog"
      ></v-closable-card-title>
      <v-card-text>
        <v-list>
          <v-list-item
            border
            v-for="fv in dialogProperty.foodValues"
            :key="`${dialogProperty.id}_${fv.id}`"
          >
            <template #prepend>
              <v-progress-circular
                size="55"
                width="5"
                :model-value="(fv.value / dialogProperty.propertyAmountTotal) * 100"
                :color="colorScale((fv.value / dialogProperty.propertyAmountTotal) * 100)"
                v-if="fv.value != null && dialogProperty.propertyAmountTotal > 0"
              >
                {{ Math.round((fv.value / dialogProperty.propertyAmountTotal) * 100) }}%
              </v-progress-circular>
              <v-progress-circular size="55" width="5" v-if="fv.value == null"
                >?</v-progress-circular
              >
            </template>
            <span class="ms-2">
            ...
            <template #append>
              <v-chip v-if="fv.value != undefined"
                >{{ $n(fv.value) }} {{ dialogProperty.unit }}</v-chip
              >
              <v-chip
                color="create"
                v-else-if="fv.missing_conversion"
                class="cursor-pointer"
                prepend-icon="$create"
              >
                {{ $t('Conversion') }}: {{ fv.missing_conversion.base_unit.name }}
                <i class="fa-solid fa-arrow-right me-1 ms-1"></i>
                {{ fv.missing_conversion.converted_unit.name }}
                <model-edit-dialog
                  model="UnitConversion"
                  @create="refreshRecipe()"
                  :item-defaults="{
                    baseAmount: 1,
                    baseUnit: fv.missing_conversion.base_unit,
                    convertedUnit: fv.missing_conversion.converted_unit,
                    food: fv.food,
                  }"
                ></model-edit-dialog>
              </v-chip>
              <v-chip
                ...
                prepend-icon="$edit"
                class="cursor-pointer"
                :to="{ name: 'ModelEditPage', params: { model: 'Recipe', id: recipe.id } }"
                v-else-if="fv.missing_unit"
              >
                {{ $t('NoUnit') }}
              </v-chip>
              <v-chip color="error" prepend-icon="$edit" class="cursor-pointer" v-else>
                ...
                <model-edit-dialog
                  model="Food"
                  :item-id="fv.food.id"
                  @update:model-value="refreshRecipe()"
                ></model-edit-dialog>
              </v-chip>
            </template>
          </v-list-item>
      ...
      <v-card-actions>
        <v-btn :to="{ name: 'PropertyEditorPage', query: { recipe: recipe.id } }">{{
          $t('Property_Editor')
        }}</v-btn>
      </v-card-actions>
    </v-card>
    ...
    if (hasRecipeProperties.value) {
      recipe.value.properties.forEach((rp) => {
        ptList.push({
          ...
          propertyAmountTotal:
            rp.propertyAmount * recipe.value.servings * (props.servings / recipe.value.servings),
          missingValue: false,
...
function refreshRecipe() {
  ...
  api
    .apiRecipeRetrieve({ id: recipe.value.id! })
    .then((r) => {
      recipe.value = r

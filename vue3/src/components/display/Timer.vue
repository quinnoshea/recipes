<template>
  ...
  <v-alert :color="timerColor" class="rounded-0" variant="tonal">
    <v-alert-title
      ...
      {{ Duration.fromMillis(durationSeconds * 1000).toFormat('hh:mm:ss') }}</v-alert-title
    >
    ...
    <template #close>
      <v-btn-group divided>
        ...
        <v-btn width="40" @click="timerRunning = !timerRunning"
          ><i class="fas fa-fw" :class="{ 'fa-pause': timerRunning, 'fa-play': !timerRunning }"></i
        ></v-btn>
...
function formatFinishTime(durationSeconds: number): string {
  ...
  const daysDifference = Math.floor(target.startOf('day').diff(now.startOf('day'), 'days').days)
  if (daysDifference >= 1) {

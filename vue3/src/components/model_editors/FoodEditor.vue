      ...
      <v-tabs-window v-model="tab">
        <v-tabs-window-item value="food">
          <v-form :disabled="loading">
            ...
            <model-select
              :label="$t('Category')"
              v-model="editingObj.supermarketCategory"
        ...
        <v-tabs-window-item value="properties">
          ...
          <v-form :disabled="loading" class="mt-5">
            ...
            >
              <template #append-inner>
                <v-btn
                  ...
                  density="compact"
                  variant="plain"
                  v-if="editingObj.fdcId == undefined"
                  @click="fdcDialog = true"
                ></v-btn>
                <v-btn
                  @click="updateFoodFdcData()"
                  icon="fa-solid fa-arrows-rotate"
                  size="small"
                  density="compact"
                  variant="plain"
                  v-if="editingObj.fdcId"
                ></v-btn>
                <v-btn
                  @click="openFdcPage(editingObj.fdcId)"
                  :href="`https://fdc.nal.usda.gov/food-details/${editingObj.fdcId}/nutrients`"
                  target="_blank"
            ...
            <v-number-input
              :label="$t('Properties_Food_Amount')"
              v-model="editingObj.propertiesFoodAmount"
              :precision="2"
            ></v-number-input>
            <model-select
              :label="$t('Properties_Food_Unit')"
              v-model="editingObj.propertiesFoodUnit"
              model="Unit"
            ></model-select>

            <properties-editor
              v-model="editingObj.properties"
              :amount-for="propertiesAmountFor"
            ></properties-editor>
        ...
        <v-tabs-window-item value="conversions">
          ...
          <v-form :disabled="loading" class="mt-5">
            <v-btn
              color="create"
              @click="unitConversions.push({ food: editingObj } as UnitConversion)"
              prepend-icon="$create"
              >{{ $t('Add') }}</v-btn
            >
            ...
            <v-card class="mt-2" border v-for="uc in unitConversions" dense>
              <v-card-title>
                ...
                <v-icon
                  ...
                  class="me-2 ms-2"
                ></v-icon>
                <span v-if="uc.convertedUnit"
                  >{{ uc.convertedAmount }} {{ uc.convertedUnit.name }}</span
                >
                <v-btn
                  color="delete"
                  class="float-right d-none d-md-block"
                  @click="deleteUnitConversion(uc, true)"
                >
                ...
                <v-btn color="edit" class="float-right d-md-none">
                  ...
                  <model-edit-dialog
                    ...
                    :item="uc"
                    @delete="deleteUnitConversion(uc, false)"
                    :disabled-fields="['food']"
                  ></model-edit-dialog>
                </v-btn>
              </v-card-title>
        ...
        <v-tabs-window-item value="hierarchy">
          ...
          <v-checkbox
            :label="$t('substitute_siblings')"
            :hint="$t('substitute_siblings_help')"
            v-model="editingObj.substituteSiblings"
            persistent-hint
          ></v-checkbox>
          <v-checkbox
            :label="$t('substitute_children')"
            :hint="$t('substitute_children_help')"
            v-model="editingObj.substituteChildren"
            persistent-hint
          ></v-checkbox>

          <ModelSelect
            model="FoodInheritField"
            v-model="editingObj.inheritFields"
            :label="$t('InheritFields')"
            :hint="$t('InheritFields_help')"
            mode="tags"
          ></ModelSelect>
          <ModelSelect
            model="FoodInheritField"
            v-model="editingObj.childInheritFields"
            :label="$t('ChildInheritFields')"
            :hint="$t('ChildInheritFields_help')"
            mode="tags"
          ></ModelSelect>
        </v-tabs-window-item>
        ...
        <v-tabs-window-item value="misc">
          <v-form :disabled="loading" class="mt-5">
            ...
            <v-checkbox
              :label="$t('OnHand')"
              :hint="$t('OnHand_help')"
              v-model="editingObj.foodOnhand"
              persistent-hint
            ></v-checkbox>
            <v-checkbox
              :label="$t('Ignore_Shopping')"
              :hint="$t('ignore_shopping_help')"
              v-model="editingObj.ignoreShopping"
              persistent-hint
            ></v-checkbox>
            <v-divider class="mt-2 mb-2"></v-divider>
            <ModelSelect
              model="Food"
              v-model="editingObj.substitute"
              :label="$t('Substitutes')"
              :hint="$t('substitute_help')"
              mode="tags"
            ></ModelSelect>

            <!-- TODO re-add reset inheritance button/api call /function (previously annotated field on food -->
            <v-text-field
              :label="$t('Open_Data_Slug')"
              :hint="$t('open_data_help_text')"
              persistent-hint
              v-model="editingObj.openDataSlug"
              disabled
            ></v-text-field>
          </v-form>
        </v-tabs-window-item>
...
const {
  ...
  editingObjChanged,
  modelClass,
} = useModelEditorFunctions<Food>('Food', emit)
...
function initializeEditor() {
  setupState(props.item, props.itemId, {
    newItemFunction: () => {
      ...
      editingObj.value.propertiesFoodUnit = {
        name:
          useUserPreferenceStore().userSettings.defaultUnit != undefined ?
            useUserPreferenceStore().userSettings.defaultUnit
          : 'g',
      } as Unit
    },
...
function loadConversions() {
  const api = new ApiApi()
  api
    .apiUnitConversionList({ foodId: editingObj.value.id })
    .then((r) => {
...
function updateFoodFdcData() {
  ...
  if (editingObj.value.fdcId) {
    saveObject().then(() => {
      loading.value = true
      api
        .apiFoodFdcCreate({ id: editingObj.value.id!, food: editingObj.value })
        .then((r) => {
          editingObj.value = r
        })
        .catch((err) => {

      ...
      <v-tabs-window v-model="tab">
        <v-tabs-window-item value="book">
          <v-form :disabled="loading">
            ...
            <v-number-input
              :label="$t('Order')"
              :hint="$t('OrderInformation')"
              v-model="editingObj.order"
            ></v-number-input>
          </v-form>
        </v-tabs-window-item>
...
const {
  ...
  editingObjChanged,
  modelClass,
} = useModelEditorFunctions<RecipeBook>('RecipeBook', emit)
    ...
    if (!duplicateFound) {
      api
        .apiRecipeBookEntryCreate({
          recipeBookEntry: { book: editingObj.value.id!, recipe: selectedRecipe.value.id! },
        })
        .then((r) => {
...
function removeRecipeFromBook(recipeBookEntry: RecipeBookEntry) {
  ...
  api
    .apiRecipeBookEntryDestroy({ id: recipeBookEntry.id! })
    .then((r) => {
      recipeBookEntries.value.splice(
        recipeBookEntries.value.findIndex((rBE) => rBE.id! == recipeBookEntry.id!),
        1
      )

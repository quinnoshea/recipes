  ...
  >
    <v-card-text>
      <v-form :disabled="loading">
        ...
        <v-checkbox
          :label="$t('SupportsDescriptionField')"
          hide-details
          v-model="editingObj.supportsDescriptionField"
        ></v-checkbox>
        ...
        <h3>{{ $t('Events') }}</h3>
        <v-checkbox
          :label="$t('ShoppingListEntry') + ' - ' + $t('Created')"
          hide-details
          v-model="editingObj.onShoppingListEntryCreatedEnabled"
        ></v-checkbox>
        <v-checkbox
          :label="$t('ShoppingListEntry') + ' - ' + $t('Updated')"
          hide-details
          v-model="editingObj.onShoppingListEntryUpdatedEnabled"
        ></v-checkbox>
        <v-checkbox
          :label="$t('ShoppingListEntry') + ' - ' + $t('Deleted')"
          hide-details
          v-model="editingObj.onShoppingListEntryDeletedEnabled"
        ></v-checkbox>
      </v-form>
    </v-card-text>
  </model-editor-base>
...
const props = defineProps({
  ...
  itemDefaults: {
    type: {} as PropType<ConnectorConfig>,
    required: false,
    default: {} as ConnectorConfig,
  },

<template>
  <model-editor-base
    ...
    :dialog="dialog"
    @save="
      saveObject().then((obj: MealPlan) => {
        useMealPlanStore().plans.set(obj.id, obj)
        loadShoppingListEntries()
      })
    "
    @delete="
      useMealPlanStore().plans.delete(editingObj.id)
      deleteObject()
    "
    @close="
      emit('close')
  ...
  >
    <v-card-text class="pa-0">
      <v-tabs v-model="tab" :disabled="loading" grow>
        ...
        <v-tab prepend-icon="$shopping" value="shopping" :disabled="!isUpdate()">{{
          $t('Shopping_list')
        }}</v-tab>
      </v-tabs>
    </v-card-text>
    ...
    <v-card-text>
      <v-tabs-window v-model="tab">
            ...
            <v-row>
              <v-col cols="12" md="6">
                <ModelSelect
                  ...
                  @update:modelValue="
                    editingObj.servings = editingObj.recipe ? editingObj.recipe.servings : 1
                  "
                ></ModelSelect>
                <!--                                <v-number-input label="Days" control-variant="split" :min="1"></v-number-input>-->
                <!--TODO create days input with +/- synced to date -->
                <recipe-card
                  :recipe="editingObj.recipe"
                  v-if="editingObj && editingObj.recipe"
                  link-target="_blank"
                ></recipe-card>
                <v-btn
                  prepend-icon="$shopping"
                  color="create"
                  class="mt-1"
                  v-if="!editingObj.shopping && editingObj.recipe && isUpdate()"
                >
                  {{ $t('Add') }}
                  <add-to-shopping-dialog
                    :recipe="editingObj.recipe"
                    :meal-plan="editingObj"
                    @created="
                      loadShoppingListEntries()
                      editingObj.shopping = true
                    "
                  ></add-to-shopping-dialog>
                </v-btn>

                <v-checkbox
                  :label="$t('AddToShopping')"
                  v-model="editingObj.addshopping"
                  hide-details
                  v-if="editingObj.recipe && !isUpdate()"
                ></v-checkbox>
              </v-col>
              <v-col cols="12" md="6">
                ...
                <v-date-input
                  ...
                  :first-day-of-week="
                    useUserPreferenceStore().deviceSettings.mealplan_startingDayOfWeek
                  "
                ...
                <v-input>
                  <v-btn-group elevation="1" class="w-100" divided border>
                    <v-btn
                      class="w-25"
                      @click="
                        adjustDateRangeLength(dateRangeValue, -1)
                        updateDate()
                      "
                    ...
                    <v-btn
                      class="w-25"
                      @click="
                        dateRangeValue = shiftDateRange(dateRangeValue, -1)
                        updateDate()
                      "
        ...
        <v-tabs-window-item value="shopping">
          <closable-help-alert
            class="mb-2"
            :text="$t('MealPlanShoppingHelp')"
          ></closable-help-alert>

          <v-row
            v-if="isUpdate()"
            dense
            style="max-height: 75vh; min-height: 30vh"
            class="overflow-y-scroll"
          >
            <v-col>
              <shopping-list-entry-input
                :loading="useShoppingStore().currentlyUpdating"
                :meal-plan="editingObj"
              ></shopping-list-entry-input>
...
function initializeDateRange() {
  if (
    editingObj.value.toDate &&
    DateTime.fromJSDate(editingObj.value.toDate)
      .diff(DateTime.fromJSDate(editingObj.value.fromDate), 'days')
      .toObject().days! >= 1
  ) {

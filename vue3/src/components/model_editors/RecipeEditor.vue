  ...
  >
      ...
      <v-tabs-window v-model="tab">
        <v-tabs-window-item value="recipe">
          <v-form :disabled="loading || fileApiLoading">
            ...
            <v-textarea
              :label="$t('Description')"
              v-model="editingObj.description"
              clearable
              counter="512"
              maxlength="512"
              rows="2"
            ...
            <v-row dense>
              <v-col cols="12" md="6">
                <v-number-input
                  :label="$t('WaitingTime')"
                  v-model="editingObj.waitingTime"
                  :step="5"
                ></v-number-input>
              </v-col>
              <v-col cols="12" md="6">
                <v-number-input
                  :label="$t('WorkingTime')"
                  v-model="editingObj.workingTime"
                  :step="5"
                ></v-number-input>
              </v-col>
              <v-col cols="12" md="6">
                <v-number-input
                  :label="$t('Servings')"
                  v-model="editingObj.servings"
                ></v-number-input>
              </v-col>
              <v-col cols="12" md="6">
                <v-text-field
                  :label="$t('ServingsText')"
                  v-model="editingObj.servingsText"
                ></v-text-field>
              </v-col>
            </v-row>
        ...
        <v-tabs-window-item value="steps">
          <v-form :disabled="loading || fileApiLoading">
            <v-row v-for="(s, i) in editingObj.steps" :key="s.id">
              <v-col>
                <step-editor
                  v-model="editingObj.steps[i]"
                  v-model:recipe="editingObj"
                  :step-index="i"
                  @delete="deleteStepAtIndex(i)"
                ></step-editor>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="text-center">
                <v-btn-group density="compact" divided border>
                  <v-btn color="success" prepend-icon="fa-solid fa-plus" @click="addStep()">{{
                    $t('Add_Step')
                  }}</v-btn>
                  <v-btn
                    color="warning"
                    @click="dialogStepManager = true"
                    :disabled="editingObj.steps.length < 2"
                  >
                    <v-icon icon="fa-solid fa-arrow-down-1-9"></v-icon>
                  </v-btn>

                  <v-btn
                    prepend-icon="fa-solid fa-maximize"
                    @click="handleSplitAllSteps"
                    :disabled="editingObj.steps.length < 1"
                    ><span v-if="!mobile">{{ $t('Split') }}</span></v-btn
                  >
                  <v-btn
                    prepend-icon="fa-solid fa-minimize"
                    @click="handleMergeAllSteps"
                    :disabled="editingObj.steps.length < 2"
        ...
        <v-tabs-window-item value="settings">
          <v-form :disabled="loading || fileApiLoading">
            ...
            <v-text-field
              :label="$t('Imported_From')"
              v-model="editingObj.sourceUrl"
            ></v-text-field>
            <v-checkbox
              :label="$t('Private_Recipe')"
              persistent-hint
              :hint="$t('Private_Recipe_Help')"
              v-model="editingObj._private"
            ></v-checkbox>
            <model-select
              mode="tags"
              model="User"
            ...
            <div class="mt-2" v-if="editingObj.filePath">
              ...
              <v-btn prepend-icon="$delete" color="error" :loading="loading"
                >{{ $t('delete_title', { type: $t('ExternalRecipe') }) }}
                <delete-confirm-dialog
                  :object-name="editingObj.filePath"
                  :model-name="$t('ExternalRecipe')"
                  @delete="deleteExternalFile()"
                ></delete-confirm-dialog>
              </v-btn>
            </div>
          </v-form>
    ...
    <v-card-text v-if="isSpaceAtRecipeLimit(useUserPreferenceStore().activeSpace)">
      <v-alert color="warning" icon="fa-solid fa-triangle-exclamation">
        {{ $t('SpaceLimitReached') }}
        <v-btn color="success" variant="flat" :to="{ name: 'SpaceSettings' }">{{
          $t('SpaceSettings')
        }}</v-btn>
      </v-alert>
    </v-card-text>
...
const {
  ...
  editingObjChanged,
  modelClass,
} = useModelEditorFunctions<Recipe>('Recipe', emit)
...
function deleteExternalFile() {
  ...
  loading.value = true
  api
    .apiRecipeDeleteExternalPartialUpdate({
      id: editingObj.value.id!,
      patchedRecipe: editingObj.value,
    })
    .then((r) => {
      editingObj.value = r
    })

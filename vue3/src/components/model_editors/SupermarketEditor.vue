          ...
          <v-row>
            <v-col cols="0" md="6">
              ...
              <draggable
                class="mt-4"
                tag="VList"
                v-model="unusedSupermarketCategories"
                handle=".drag-handle"
                item-key="id"
                group="categories"
                :move="trackMovedItem"
                ...
                <template #footer>
                  ...
                  >
                    ...
                    <model-edit-dialog
                      ...
                      @create="(args: SupermarketCategory) => supermarketCategories.push(args)"
                    ></model-edit-dialog>
                  </v-list-item>
                </template>
              </draggable>
...
const {
  ...
  editingObjChanged,
  modelClass,
} = useModelEditorFunctions<Supermarket>('Supermarket', emit)
...
const unusedSupermarketCategories = computed(() => {
  ...
  supermarketCategories.value.forEach((sc) => {
    if (
      editingObjectSupermarketCategoriesRelations.value.findIndex((e) => e.category.id == sc.id) ==
      -1
    ) {
      sCR.push({
  ...
  if (!preventSort.value) {
    editingObjectSupermarketCategoriesRelations.value.forEach((sc, index) => {
      if (index >= startIndex) {
        ...
        api
          .apiSupermarketCategoryRelationUpdate({ id: sc.id!, supermarketCategoryRelation: sc })
          .catch((err: any) => {
...
function addCategoryRelation(sCR: SupermarketCategoryRelation) {
  ...
  let relationIndex = editingObjectSupermarketCategoriesRelations.value.findIndex(
    (e) => e.category.id == sCR.category.id
  )
  if (relationIndex != -1) {
    if (
      relationIndex != 0 &&
      editingObjectSupermarketCategoriesRelations.value[relationIndex - 1]
    ) {
    ...
    sCR.order =
      editingObjectSupermarketCategoriesRelations.value[
        editingObjectSupermarketCategoriesRelations.value.length - 1
      ].order! + 1
  }

  api
    .apiSupermarketCategoryRelationCreate({ supermarketCategoryRelation: sCR })
    .then((r) => {

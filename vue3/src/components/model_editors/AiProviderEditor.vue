    ...
    <v-card-text>
      <v-form :disabled="loading">
        ...
        <v-checkbox
          ...
          v-if="useUserPreferenceStore().userSettings.user.isSuperuser"
        ></v-checkbox>
        ...
        <v-combobox
          :label="$t('Model')"
          :items="aiModels"
          v-model="editingObj.modelName"
          hide-details
        >
        </v-combobox>

        <p class="mt-2 mb-2">
          You can use any model that
          <a href="https://docs.litellm.ai/docs/providers" target="_blank">LiteLLM supports</a>, the
          list just contains some of the most commonly used models.
        </p>
...
const {
  ...
  editingObjChanged,
  modelClass,
} = useModelEditorFunctions<AiProvider>('AiProvider', emit)

      ...
      <v-form :disabled="loading || fileApiLoading">
        ...
        <v-row>
          <v-col cols="12" md="6">
            <v-file-upload
              v-model="file"
              @update:modelValue="updateUserFileName"
        ...
        <v-alert
          ...
          v-if="
            !loading &&
            !fileApiLoading &&
            Object.keys(editingObj).length > 0 &&
            Number(editingObj.fileSizeKb)
          "
        >
          <p v-if="Number(editingObj.fileSizeKb)">
            {{ $n(editingObj.fileSizeKb / 1000) }} MB <br />
          </p>
          <p v-if="editingObj.createdBy">{{ editingObj.createdBy.displayName }}</p>
          <p v-if="editingObj.createdAt">
...
const {
  ...
  editingObjChanged,
  modelClass,
} = useModelEditorFunctions<UserFile>('UserFile', emit)
...
function saveFile() {
  ...
  createOrUpdateUserFile(editingObj.value.name, file.value, editingObj.value.id)
    .then((r) => {
      editingObj.value = r

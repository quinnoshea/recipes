<template>
  <v-form>
    ...
    <v-data-table
      :items="spaceUserSpaces"
      :headers="userTableHeaders"
      density="compact"
      :hide-default-footer="spaceUserSpaces.length < 10"
      class="mt-3"
    >
      ...
      <template #item.edit="{ item }">
        <v-btn
          color="edit"
          size="small"
          v-if="item.user.id != useUserPreferenceStore().activeSpace.createdBy.id"
        >
          <v-icon icon="$edit"></v-icon>
          <model-edit-dialog
            model="UserSpace"
            :item="item"
            @delete="deleteUserSpace(item)"
            class="mt-2"
          ></model-edit-dialog>
        </v-btn>
    ...
    <p class="text-h6 mt-3">
      ...
      <v-btn size="small" class="float-right" prepend-icon="$create" color="create">
        ...
        <model-edit-dialog
          ...
          @create="(item) => spaceInviteLinks.push(item)"
          class="mt-2"
        ></model-edit-dialog>
      </v-btn>
    </p>
    ...
    <v-data-table
      :items="spaceInviteLinks"
      :headers="inviteTableHeaders"
      density="compact"
      :hide-default-footer="spaceInviteLinks.length < 10"
    >

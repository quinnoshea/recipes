<template>
  <v-form>
    ...
    <v-form v-if="searchPreferences">
      <v-select
        v-model="searchPreferences.search"
        :items="searchMethods"
        :label="$t('Method')"
      ></v-select>

      <v-checkbox
        v-model="searchPreferences.lookup"
        :label="$t('Fuzzy')"
        persistent-hint
        :hint="$t('FuzzySearchHelp')"
        class="mb-4"
      ></v-checkbox>
      <v-number-input
        v-model="searchPreferences.trigramThreshold"
        :precision="2"
        :min="0.01"
        :max="1"
        :step="0.1"
        :label="$t('TrigramThreshold')"
        persistent-hint
        :hint="$t('TrigramThresholdHelp')"
      ></v-number-input>

      <ModelSelect
        model="SearchFields"
        mode="tags"
        v-model="searchPreferences.unaccent"
        :label="$t('IgnoreAccents')"
        :hint="$t('IgnoreAccentsHelp')"
      ></ModelSelect>
      <ModelSelect
        model="SearchFields"
        mode="tags"
        v-model="searchPreferences.icontains"
        :label="$t('PartialMatch')"
        :hint="$t('PartialMatchHelp')"
      ></ModelSelect>
      <ModelSelect
        model="SearchFields"
        mode="tags"
        v-model="searchPreferences.istartswith"
        :label="$t('StartsWith')"
        :hint="$t('StartsWithHelp')"
      ></ModelSelect>
      <ModelSelect
        model="SearchFields"
        mode="tags"
        v-model="searchPreferences.fulltext"
        :label="$t('Fulltext')"
        :hint="$t('FulltextHelp')"
      ></ModelSelect>
      <ModelSelect
        model="SearchFields"
        mode="tags"
        v-model="searchPreferences.trigram"
        :label="$t('Fuzzy')"
        :hint="$t('FuzzySearchHelp')"
      ></ModelSelect>
...
function updateSearchSettings() {
  ...
  if (searchPreferences.value != undefined) {
    api
      .apiSearchPreferencePartialUpdate({
        patchedSearchPreference: searchPreferences.value,
        user: useUserPreferenceStore().userSettings.user.id,
      })
      .then((r) => {

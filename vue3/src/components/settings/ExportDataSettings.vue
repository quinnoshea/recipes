<template>
  ...
  <v-form class="mt-2">
    ...
    <v-checkbox
      :label="$t('AllRecipes')"
      v-model="allRecipes"
      :disabled="selectedRecipes.length > 0 || selectedFilter != null"
    ></v-checkbox>
    <ModelSelect
      model="Recipe"
      mode="tags"
      v-model="selectedRecipes"
      :disabled="allRecipes || selectedFilter != null"
    ></ModelSelect>
    <ModelSelect
      model="CustomFilter"
      mode="single"
      v-model="selectedFilter"
      :disabled="selectedRecipes.length > 0 || allRecipes"
    ></ModelSelect>

    <v-btn
      @click="doExport()"
      :loading="loading"
      :disabled="selectedRecipes.length == 0 && selectedFilter == null && !allRecipes"
      >{{ $t('Export') }}</v-btn
    >
    ...
    <template v-if="exportLog.id">
      ...
      <p>{{ $t('Recipes') }}: {{ exportLog.exportedRecipes }}</p>

      <v-btn
        color="success"
        :href="useDjangoUrls().getDjangoUrl(`export-file/${exportLog.id!}`)"
        class="mt-2"
        :disabled="exportLog.running"
        >{{ $t('Download') }}</v-btn
      >
...
function doExport() {
  ...
  api
    .apiExportCreate({
      exportRequest: {
        all: allRecipes.value,
        type: exportType.value,
        recipes: selectedRecipes.value,
        customFilter: selectedFilter.value,
      },
    })
    .then((r) => {
      exportLog.value = r

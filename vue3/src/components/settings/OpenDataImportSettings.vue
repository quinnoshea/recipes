<template>
  ...
  <v-btn
    href="https://github.com/TandoorRecipes/open-tandoor-data"
    target="_blank"
  ...
  <v-select
    ...
    :label="$t('Language')"
    class="mt-2"
    v-model="requestData.selectedVersion"
    :loading="loading"
  ></v-select>
  ...
  <v-row v-if="requestData.selectedVersion">
    <v-col>
      <v-checkbox
        :label="$t('Update_Existing_Data')"
        v-model="requestData.updateExisting"
        hide-details
      ></v-checkbox>
      <v-checkbox
        :label="$t('Use_Metric')"
        v-model="requestData.useMetric"
        hide-details
      ></v-checkbox>
          ...
          <tr v-for="d in metadata.datatypes">
            ...
            <td>
              <template v-if="responseData[d]">
                <i class="fas fa-plus-circle"></i> {{ responseData[d].totalCreated }}
                {{ $t('Created') }} <br />
                <i class="fas fa-pencil-alt"></i> {{ responseData[d].totalUpdated }}
                {{ $t('Updated') }} <br />
                <i class="fas fa-forward"></i> {{ responseData[d].totalUntouched }}
                {{ $t('Unchanged') }} <br />
                <i class="fas fa-exclamation-circle"></i> {{ responseData[d].totalErrored }}
                {{ $t('Error') }}
              </template>
            </td>
          </tr>
...
function importOpenData() {
  ...
  api
    .apiImportOpenDataCreate({ importOpenData: requestData.value })
    .then((r) => {
      console.log(r)

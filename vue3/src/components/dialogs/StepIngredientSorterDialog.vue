<template>
  <v-dialog v-model="dialog" :max-width="mobile ? '100vw' : '25vw'" :fullscreen="mobile">
    <v-card>
      <v-closable-card-title
        :title="$t('Move')"
        v-model="dialog"
        :sub-title="ingredientToString(step.ingredients[editingIngredientIndex])"
      ></v-closable-card-title>
      <v-card-text>
        <template v-if="step.ingredients.length > 1">
          ...
          <v-btn
            ...
            @click="moveIngredient(editingIngredientIndex, props.stepIndex, 0)"
            >{{ $t('First') }}</v-btn
          >
          <v-btn
            block
            ...
            class="mt-1"
            @click="
              moveIngredient(editingIngredientIndex, props.stepIndex, editingIngredientIndex - 1)
            "
          >
            {{ $t('Up') }}
          </v-btn>
          <v-btn
            block
            :disabled="editingIngredientIndex + 1 == step.ingredients.length"
            class="mt-1"
            @click="
              moveIngredient(editingIngredientIndex, props.stepIndex, editingIngredientIndex + 1)
            "
          ...
          <v-btn
            ...
            class="mt-1"
            @click="
              moveIngredient(editingIngredientIndex, props.stepIndex, step.ingredients.length - 1)
            "
            >{{ $t('Last') }}
          </v-btn>
        </template>
        ...
        <v-btn
          ...
          :disabled="i == props.stepIndex"
          class="mt-1"
          @click="moveIngredient(editingIngredientIndex, i, recipe.steps[i].ingredients.length)"
          >{{ i + 1 }} <span v-if="'name' in s">{{ s.name }}</span>
        </v-btn>
      </v-card-text>
      <v-card-actions>

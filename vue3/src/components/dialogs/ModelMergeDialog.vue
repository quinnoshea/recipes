    ...
    <v-card :loading="loading">
      ...
      <v-card-text>
        {{
          $t('merge_selection', {
            source: sourceNames,
            type: $t(genericModel.model.localizationKey),
          })
        }}
              ...
              <v-list-item border v-for="item in sourceItems">
                ...
                <template #append>
                  <v-icon
                    ...
                    color="error"
                    variant="tonal"
                    v-if="failedItems.includes(item)"
                  ></v-icon>
                  <v-icon
                    icon="fa-solid fa-check"
                    color="success"
                    variant="tonal"
                    v-else-if="updatedItems.includes(item)"
                  ></v-icon>
                  <v-icon
        ...
        <v-checkbox
          ...
          persistent-hint
          v-if="genericModel.model.mergeAutomation"
        ></v-checkbox>
      </v-card-text>
      <v-card-actions>
        <v-btn :disabled="loading" @click="dialog = false">{{ $t('Cancel') }}</v-btn>
        <v-btn color="warning" @click="mergeModel()" :loading="loading" :disabled="!target">{{
          $t('Merge')
        }}</v-btn>
      </v-card-actions>
    </v-card>

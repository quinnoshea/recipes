  ...
  <v-dialog :fullscreen="mobile" v-model="showDialog" max-width="500px">
    <v-card>
      <v-closable-card-title
        :title="shoppingListFood.food.name"
        v-model="showDialog"
      ></v-closable-card-title>
      ...
      <v-card-text class="pt-0 pr-4 pl-4">
        ...
        <model-select
          ...
          @update:modelValue="categoryUpdate"
          allow-create
        ></model-select>
        ...
        <v-row>
          <v-col class="pr-0">
            <v-btn
              ...
              stacked
              @click="
                useShoppingStore().delayEntries(entriesList, !isShoppingLineDelayed, true)
                showDialog = false
              "
            >
          ...
          <v-col>
            <v-btn
              ...
              stacked
              @click="
                useShoppingStore().setFoodIgnoredState(entriesList, true, true)
                showDialog = false
              "
            >
        ...
        <v-row>
          <v-col class="pr-0 pt-0">
            <v-btn
              ...
              :to="{
                name: 'ModelEditPage',
                params: { model: 'Food', id: shoppingListFood?.food.id! },
              }"
              target="_blank"
              block
              stacked
          ...
          <template v-for="[i, e] in shoppingListFood.entries" :key="e.id">
            <v-list-item border class="mt-1">
              ...
              <v-list-item-subtitle v-if="e.completedAt">
                ...
                {{ $t('Completed') }}
                {{ DateTime.fromJSDate(e.completedAt).toLocaleString(DateTime.DATETIME_SHORT) }}
              ...
              <v-list-item-subtitle v-if="e.listRecipe && e.listRecipeData.mealplan">
                {{ e.listRecipeData.mealPlanData.mealType.name }}
                {{
                  DateTime.fromJSDate(e.listRecipeData.mealPlanData.fromDate).toLocaleString(
                    DateTime.DATE_SHORT
                  )
                }}
              </v-list-item-subtitle>
              <v-list-item-subtitle>
                ...
                {{ DateTime.fromJSDate(e.createdAt).toLocaleString(DateTime.DATETIME_SHORT) }}
              </v-list-item-subtitle>
              <v-list-item-subtitle v-if="isDelayed(e)" class="text-info font-weight-bold">
                {{ $t('PostponedUntil') }}
                {{ DateTime.fromJSDate(e.delayUntil!).toLocaleString(DateTime.DATETIME_SHORT) }}
              </v-list-item-subtitle>
              ...
              <v-btn-group divided border>
                ...
                <v-btn color="edit" icon="$edit" v-if="!e.ingredient">
                  ...
                  <model-edit-dialog
                    ...
                    :item="e"
                    @delete="
                      useShoppingStore().entries.delete(e.id!)
                      shoppingListFood.entries.delete(e.id!)
                    "
                    @save="
                      (args: ShoppingListEntry) => {
                        useShoppingStore().entries.set(e.id!, args)
                ...
                <v-btn
                  color="edit"
                  icon="$recipes"
                  v-if="e.listRecipe && e.listRecipeData.recipe && e.ingredient"
...
function categoryUpdate(category: SupermarketCategory) {
  ...
  api
    .apiFoodUpdate({ id: shoppingListFood.value.food.id, food: shoppingListFood.value.food })
    .then((r) => {
...
function addEntryForFood() {
  useShoppingStore()
    ...
    .then((r: ShoppingListEntry | undefined) => {
      if (r != undefined) {
...
function updateEntryAmount(entry: ShoppingListEntry) {
  ...
  api
    .apiShoppingListEntryPartialUpdate({
      id: entry.id!,
      patchedShoppingListEntry: { amount: entry.amount } as PatchedShoppingListEntry,
    })
    .then((r) => {})
    .catch((err) => {
      useMessageStore().addError(ErrorMessageType.UPDATE_ERROR, err)

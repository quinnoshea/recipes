  ...
  <v-dialog max-width="900" v-model="dialog">
    <v-card>
      ...
      <v-card-text>
        ...
        <v-select
          multiple
          v-model="fdcDataTypeSelection"
          :items="fdcDataTypeOptions"
          chips
        ></v-select>

        <v-list>
          <v-list-item
            v-for="f in fdcQueryResults?.foods"
            :title="f.description"
            @click="
              dialog = false
              emit('selected', f.fdcId)
            "
          >
            <v-list-item-subtitle
              >{{ f.dataType }} <v-chip size="small">{{ f.fdcId }}</v-chip></v-list-item-subtitle
            >
          </v-list-item>
        </v-list>
      </v-card-text>
...
function fdcSearch() {
  ...
  loading.value = true
  api
    .apiFdcSearchRetrieve({ query: query.value, dataType: fdcDataTypeSelection.value.join(',') })
    .then((r) => {

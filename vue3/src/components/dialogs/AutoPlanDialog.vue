    ...
    <v-card :loading="loading">
      ...
      <v-card-text>
        <v-form>
          <model-select
            model="MealType"
            v-model="autoMealPlan.mealTypeId"
            :object="false"
          ></model-select>
          <model-select
            model="Keyword"
            v-model="autoMealPlan.keywordIds"
            mode="tags"
            :object="false"
          ></model-select>
          ...
          <v-checkbox
            v-model="autoMealPlan.addshopping"
      ...
      <v-card-actions>
        ...
        <v-btn
          color="create"
          prepend-icon="fa-solid fa-person-running"
          @click="doAutoPlan()"
          :loading="loading"
          >{{ $t('Create') }}</v-btn
        >
...
function doAutoPlan() {
  ...
  console.log(
    'requesting auto plan from ',
    autoMealPlan.value.startDate,
    ' to ',
    autoMealPlan.value.endDate
  )

  api
    .apiAutoPlanCreate({ autoMealPlan: autoMealPlan.value })
    .then((r) => {
      dialog.value = false

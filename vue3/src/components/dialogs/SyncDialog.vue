    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <template v-if="syncLog">
          <v-chip label v-if="syncLog.status == 'SUCCESS'" color="success">{{
            $t('Success')
          }}</v-chip>
          ...
          <v-textarea
            ...
            max-rows="10"
            v-model="syncLog.msg"
            :hint="DateTime.fromJSDate(syncLog.createdAt).toLocaleString(DateTime.DATETIME_SHORT)"
            persistent-hint
            readonly
          ></v-textarea>
        </template>
      ...
      <v-card-actions>
        <v-btn @click="performSync()" color="create" :loading="loading" v-if="!syncLog">{{
          $t('Import')
        }}</v-btn>
        <v-btn
          :to="{ name: 'ModelListPage', params: { model: 'RecipeImport' } }"
          color="primary"
          :loading="loading"
          v-if="syncLog"
          >{{ $t('View') }}</v-btn
...
function performSync() {
  ...
  api
    .apiSyncQuerySyncedFolderCreate({ id: props.sync.id!, sync: props.sync })
    .then((r) => {
      syncLog.value = r
    })
    .catch((err) => {

  ...
  <v-dialog activator="parent" max-width="600px" v-model="dialog">
    <v-card :loading="loading">
      <v-closable-card-title
        :title="$t('Add_Servings_to_Shopping', { servings: servings })"
        v-model="dialog"
      ></v-closable-card-title>
      <v-card-text>
              ...
              <v-table density="compact">
                <tbody>
                  ...
                  >
                    ...
                    <td
                      style="width: 1%; text-wrap: nowrap"
                      class="pr-1"
                      v-html="
                        calculateFoodAmount(
                          e.amount,
                          ingredientFactor,
                          useUserPreferenceStore().userSettings.useFractions
                        )
                      "
                    ></td>
                    <td style="width: 1%; text-wrap: nowrap" class="pr-1">
                      <template v-if="e.unit">
                        {{ ingredientToUnitString(e.ingredient, ingredientFactor) }}</template
                      >
                    </td>
                    <td>
                      <template v-if="e.food">
                        {{ ingredientToFoodString(e.ingredient, ingredientFactor) }}</template
                      >
                    </td>
                  </tr>
                </tbody>
      ...
      <v-card-actions>
        <v-btn
          ...
          prepend-icon="$create"
          color="create"
          @click="createShoppingListRecipe()"
          :disabled="loading"
          >{{ $t('Add_to_Shopping') }}</v-btn
        >
      </v-card-actions>
    </v-card>
  </v-dialog>
...
import {
  ...
  RecipeFlat,
  RecipeOverview,
  type ShoppingListEntryBulkCreate,
  ShoppingListRecipe,
} from '@/openapi'
...
function loadRecipeData() {
  ...
  let recipeRequest = api
    .apiRecipeRetrieve({ id: props.recipe.id! })
    .then((r) => {
      recipe.value = r
          ...
          step.ingredients.forEach((ingredient) => {
            if (!ingredient.isHeader) {
              dialogRecipe.entries.push({
                ...
                checked:
                  ingredient.food ?
                    !(ingredient.food.ignoreShopping || ingredient.food.foodOnhand)
                  : true,
              })
            }
          })
...
function createShoppingListRecipe() {
    ...
    dialogRecipe.entries.forEach((entry) => {
      if (entry.checked) {
        shoppingListEntries.entries.push({
          amount:
            entry.amount * (servings.value / (recipe.value.servings ? recipe.value.servings : 1)),
  ...
  api
    .apiShoppingListRecipeCreate({ shoppingListRecipe: shoppingListRecipe })
    .then((slr) => {
      api
        .apiShoppingListRecipeBulkCreateEntriesCreate({
          id: slr.id!,
          shoppingListEntryBulkCreate: shoppingListEntries,
        })
        .then((r) => {

    ...
    <template v-slot:default="{ isActive }">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-btn-toggle class="mt-2" v-model="typeFilter" variant="outlined" divided multiple>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn @click="useMessageStore().deleteAllMessages()" color="error">{{
            $t('Delete_All')
          }}</v-btn>
  ...
  <v-dialog v-model="showDetailDialog" max-width="50vw">
    <v-card>
      <v-card-title>
        {{ $t('Created') }}
        <small>{{
          DateTime.fromSeconds(detailItem.createdAt).toLocaleString(DateTime.DATETIME_MED)
        }}</small>
      </v-card-title>
      <v-divider></v-divider>
      <v-card-text>
        ...
        <pre style="white-space: pre-wrap" v-if="detailItem.data != null">{{
          detailItem.data
        }}</pre>
      </v-card-text>
      ...
      <v-card-actions>
        ...
        <v-btn text="Close Dialog" @click="showDetailDialog = false"></v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
function addTestMessage() {
  ...
  useMessageStore().addMessage(
    ...
    {
      title: 'Test',
      text: `Lorem Ipsum Lorem Ipsum Lorem Ipsum LINEBREAK \n Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum ${
        Math.random() * 1000
      }`,
    },
    5000,
    { json: 'data', msg: 'whatever', data: 1 }
  )
}
</script>

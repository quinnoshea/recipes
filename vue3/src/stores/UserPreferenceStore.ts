...
export const useUserPreferenceStore = defineStore('user_preference_store', () => {
   ...
   */
  let deviceSettings = useStorage(DEVICE_SETTINGS_KEY, getDefaultDeviceSettings(), localStorage, {
    mergeDefaults: true,
  })
  /**
  ...
  function updateUserSettings() {
    ...
    api
      .apiUserPreferencePartialUpdate({
        user: userSettings.value.user.id!,
        patchedUserPreference: userSettings.value,
      })
      .then((r) => {
  ...
  function switchSpace(space: Space) {
    ...
    api
      .apiSwitchActiveSpaceRetrieve({ spaceId: space.id! })
      .then((r) => {
        loadActiveSpace()

...
export const useShoppingStore = defineStore(_STORE_ID, () => {
  ...
  const getEntriesByGroup = computed(() => {
    ...
    if (
      ...
      useUserPreferenceStore().deviceSettings.shopping_selected_supermarket.categoryToSupermarket.forEach(
        (cTS) => {
          structure.categories.set(cTS.category.name, {
            name: cTS.category.name,
            foods: new Map<number, IShoppingListFood>(),
          } as IShoppingListCategory)
        }
      )
    }
    ...
    entries.value.forEach((shoppingListEntry) => {
      if (shoppingListEntry.listRecipe && shoppingListEntry.listRecipeData.mealplan == mealPlanId) {
        items.push({
          ...
          entries: new Map<number, ShoppingListEntry>().set(
  ...
  function hasFailedItems() {
    for (let i in itemCheckSyncQueue.value) {
      if (
        itemCheckSyncQueue.value[i]['status'] === 'syncing_failed_before' ||
        itemCheckSyncQueue.value[i]['status'] === 'waiting_failed_before'
      ) {
        return true
      }
    }
  ...
  function recLoadShoppingListEntries(requestParameters: ApiShoppingListEntryListRequest) {
    let api = new ApiApi()
    api
      .apiShoppingListEntryList(requestParameters)
      .then((r) => {
        r.results.forEach((e) => {
  ...
  function autoSync() {
    if (!currentlyUpdating.value && autoSyncHasFocus.value && !hasFailedItems()) {
      ...
      api
        .apiShoppingListEntryList({ updatedAfter: autoSyncLastTimestamp.value })
        .then((r) => {
  ...
  function createObject(object: ShoppingListEntry, undo: boolean) {
    const api = new ApiApi()
    return api
      .apiShoppingListEntryCreate({ shoppingListEntry: object })
      .then((r) => {
  ...
  function updateObject(object: ShoppingListEntry) {
    ...
    return api
      .apiShoppingListEntryUpdate({ id: object.id!, shoppingListEntry: object })
      .then((r) => {
        entries.value.set(r.id!, r)
      })
      .catch((err) => {
  ...
  function deleteObject(object: ShoppingListEntry, undo: boolean) {
    const api = new ApiApi()
    return api
      .apiShoppingListEntryDestroy({ id: object.id! })
      .then((r) => {
  ...
  function getAssociatedRecipes(): ShoppingListRecipe[] {
    ...
    entries.value.forEach((e) => {
      if (
        e.listRecipe != null &&
        recipes.findIndex((x) => x.id == e.listRecipe) == -1 &&
        isEntryVisible(e, useUserPreferenceStore().deviceSettings)
      ) {
  ...
  function updateEntryInStructure(structure: IShoppingList, entry: ShoppingListEntry) {
    ...
    if (
      group == ShoppingGroupingOptions.CATEGORY &&
      entry.food != null &&
      entry.food.supermarketCategory != null
    ) {
      ...
      if (entry.listRecipeData.recipeData != null) {
        ...
        if (entry.listRecipeData.mealPlanData != null) {
          groupingKey +=
            ' - ' +
            entry.listRecipeData.mealPlanData.mealType.name +
            ' - ' +
            DateTime.fromJSDate(entry.listRecipeData.mealPlanData.fromDate).toLocaleString(
              DateTime.DATE_SHORT
            )
        }
      }
  ...
  function _replaySyncQueue() {
    if (navigator.onLine || document.location.href.includes('localhost')) {
      ...
      itemCheckSyncQueue.value.forEach((entry, index) => {
        ...
        let p = api
          .apiShoppingListEntryBulkCreate({ shoppingListEntryBulk: entry }, {})
          .then((r) => {
  ...
  function deleteShoppingListRecipe(shopping_list_recipe_id: number) {
    ...
    api
      .apiShoppingListRecipeDestroy({ id: shopping_list_recipe_id })
      .then((x) => {

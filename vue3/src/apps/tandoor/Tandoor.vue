<template>
  <v-app>
    ...
    <v-app-bar
      :color="
        useUserPreferenceStore().activeSpace.navBgColor ?
          useUserPreferenceStore().activeSpace.navBgColor
        : useUserPreferenceStore().userSettings.navBgColor
      "
      flat
      density="comfortable"
      v-if="useUserPreferenceStore().isAuthenticated"
    ...
    >
      <router-link :to="{ name: 'StartPage', params: {} }">
        <v-img
          ...
          class="ms-2"
          v-if="
            useUserPreferenceStore().userSettings.navShowLogo &&
            !useUserPreferenceStore().activeSpace.navLogo
          "
        ></v-img>
        <v-img
          ...
          v-if="
            useUserPreferenceStore().userSettings.navShowLogo &&
            useUserPreferenceStore().activeSpace.navLogo != undefined
          "
        ></v-img>
      </router-link>
        ...
        <v-menu activator="parent">
          <v-list density="compact">
            ...
            <component
              ...
              :="item"
              :key="item.title"
              v-for="item in useNavigation().getUserNavigation()"
            ></component>
          </v-list>
        </v-menu>
      </v-avatar>
    ...
    <v-app-bar
      ...
      density="compact"
      v-if="
        useUserPreferenceStore().isAuthenticated &&
        useUserPreferenceStore().activeSpace.maxRecipes == 10 &&
        useUserPreferenceStore().serverSettings.hosted
      "
    >
      <p class="text-center w-100">
        {{ $t('HostedFreeVersion') }}
        <v-btn color="success" variant="flat" href="https://tandoor.dev/manage">{{
          $t('UpgradeNow')
        }}</v-btn>
      </p>
    </v-app-bar>
    <v-app-bar
      ...
      v-if="
        useUserPreferenceStore().isAuthenticated &&
        isSpaceAboveLimit(useUserPreferenceStore().activeSpace)
      "
    >
      <p class="text-center w-100">
    ...
    <v-navigation-drawer v-if="lgAndUp && useUserPreferenceStore().isAuthenticated && !isPrintMode">
      <v-list nav>
        ...
        <component
          ...
          :="item"
          :key="item.title"
          v-for="item in useNavigation().getNavigationDrawer()"
        ></component>
      ...
      <template #append>
        <v-list nav>
          <v-list-item
            prepend-icon="fas fa-sliders"
            :title="$t('Settings')"
            :to="{ name: 'SettingsPage', params: {} }"
          ></v-list-item>
        ...
        <v-bottom-sheet activator="parent" close-on-content-click>
          <v-list nav>
            <component
              ...
              :="item"
              :key="item.title"
              v-for="item in useNavigation().getBottomNavigation()"
            ></component>
          </v-list>
        </v-bottom-sheet>
      </v-btn>

...
export function ingredientToFoodString(ingredient: Ingredient, ingredientFactor: number) {
  if (ingredient.food) {
    return pluralString(
      ingredient.food,
      ingredient.noAmount ? 0 : ingredient.amount * ingredientFactor,
      ingredient.alwaysUsePluralFood
    )
  } else {
    return ''
  }
...
export function ingredientToUnitString(ingredient: Ingredient, ingredientFactor: number) {
  if (ingredient.unit) {
    if (
      ...
      ingredient.unit.pluralName == undefined ||
      ingredient.noAmount
    ) {
...
export function getRecipeIngredients(
  recipe: Recipe,
  t: any,
  options: { showStepHeaders: boolean } = { showStepHeaders: false }
) {
  let ingredients = [] as Ingredient[]
  recipe.steps.forEach((step, index) => {
    if (
      step.showAsHeader &&
      options.showStepHeaders &&
      recipe.steps.length > 1 &&
      (step.ingredients.length > 0 || step.name != '')
    ) {
      ingredients.push({
        amount: 0,
        unit: null,
